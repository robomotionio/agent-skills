...
// node_modules/streamdown/dist/chunk-YOKDWASO.js
import { createContext, memo, useContext, useMemo, lazy, isValidElement, useLayoutEffect, useId, useRef, useEffect, cloneElement, createElement, useState, useCallback, Suspense } from "react";
    ...
    switch (type) {
      ...
      case OBJECT: {
        ...
        for (const [key, index3] of value) {
          const k3 = unpair(key), value2 = unpair(index3);
          if (k3 === "__proto__") defineProperty(object, k3, {
            value: value2,
            configurable: true,
            enumerable: true,
            writable: true
          });
          else object[k3] = value2;
        }
        return object;
      }
      case DATE:
...
function patch(state, from, to2) {
  if ("sourceCodeLocation" in from && from.sourceCodeLocation && state.file) {
    const position4 = createLocation(state, to2, from.sourceCodeLocation);
    if (position4) {
      state.location = true;
...
function zwitch(key, options) {
  ...
  function one5(value, ...parameters) {
    let fn3 = one5.invalid;
    const handlers2 = one5.handlers;
    if (value && own2.call(value, key)) {
      const id = String(value[key]);
      fn3 = own2.call(handlers2, id) ? handlers2[id] : one5.unknown;
    }
    if (fn3) {
...
function isMathMLTextIntegrationPoint(tn3, ns) {
  return ns === NS.MATHML && (tn3 === TAG_ID.MI || tn3 === TAG_ID.MO || tn3 === TAG_ID.MN || tn3 === TAG_ID.MS || tn3 === TAG_ID.MTEXT);
}
function isHtmlIntegrationPoint(tn3, ns, attrs) {
  if (ns === NS.MATHML && tn3 === TAG_ID.ANNOTATION_XML) {
  ...
  return ns === NS.SVG && (tn3 === TAG_ID.FOREIGN_OBJECT || tn3 === TAG_ID.DESC || tn3 === TAG_ID.TITLE);
}
function isIntegrationPoint(tn3, ns, attrs, foreignNS) {
  return (!foreignNS || foreignNS === NS.HTML) && isHtmlIntegrationPoint(tn3, ns, attrs) || (!foreignNS || foreignNS === NS.MATHML) && isMathMLTextIntegrationPoint(tn3, ns);
}
...
var P = (n, r2, e) => r2 in n ? dn(n, r2, { enumerable: true, configurable: true, writable: true, value: e }) : n[r2] = e;
var I = (n, r2) => {
  for (var e in r2 || (r2 = {})) mn.call(r2, e) && P(n, e, r2[e]);
  if (M) for (var e of M(r2)) pn.call(r2, e) && P(n, e, r2[e]);
  return n;
};
var b = (n, r2) => gn(n, hn(r2));
var kn = (n) => {
  let r2 = new Uint8Array(n.length + 1), e = false, i = false, o = 0;
  for (; o < n.length; ) {
    if (n[o] === "\\" && o + 1 < n.length && n[o + 1] === "`") {
      let s2 = e || i ? 1 : 0;
      r2[o + 1] = s2, r2[o + 2] = s2, o += 2;
      continue;
    }
    if (n.substring(o, o + 3) === "```") {
      i = !i;
      let s2 = e || i ? 1 : 0, t = Math.min(o + 3, n.length);
      for (let a = o + 1; a <= t; a += 1) r2[a] = s2;
      o = t;
      continue;
    }
    !i && n[o] === "`" && (e = !e), r2[o + 1] = e || i ? 1 : 0, o += 1;
  }
  return r2;
};
var $2 = null;
var c = (n, r2) => {
  let e = $2;
  return (e === null || e.text !== n) && (e = { text: n, lookup: kn(n) }, $2 = e), e.lookup[Math.min(r2, n.length)] === 1;
};
var In = (n, r2) => {
  let e = n.substring(r2, r2 + 3) === "```", i = r2 > 0 && n.substring(r2 - 1, r2 + 2) === "```", o = r2 > 1 && n.substring(r2 - 2, r2 + 1) === "```";
  return e || i || o;
};
var E = (n) => {
  let r2 = 0;
...
var bn = /^(\s*(?:[-*+]|\d+[.)]) +)>(=?\s*[$]?\d)/gm;
var y = (n) => !n || typeof n != "string" || !n.includes(">") ? n : n.replace(bn, (r2, e, i, o) => c(n, o) ? r2 : `${e}\\>${i}`);
var N = /(\*\*)([^*]*\*?)$/;
var R = /(__)([^_]*?)$/;
var U = /(\*\*\*)([^*]*?)$/;
var W = /(\*)([^*]*?)$/;
...
var f = (n) => {
  ...
  return r2 >= 48 && r2 <= 57 || r2 >= 65 && r2 <= 90 || r2 >= 97 && r2 <= 122 || r2 === 95 ? true : w.test(n);
};
var Y = (n, r2) => {
  let e = 1;
...
var T = (n, r2) => {
  let e = 1;
  for (let i = r2 + 1; i < n.length; i += 1) if (n[i] === "[") e += 1;
  else if (n[i] === "]" && (e -= 1, e === 0)) return i;
  return -1;
};
var An = (n) => n === "inlineLatex" || n === "blockLatex";
var Tn = (n, r2) => r2 === "[" && n === "none" ? "blockLatex" : r2 === "]" && n === "blockLatex" ? "none" : r2 === "(" && n === "none" ? "inlineLatex" : r2 === ")" && n === "inlineLatex" ? "none" : null;
var Ln = (n, r2) => r2 ? n === "blockDollar" ? "none" : "blockDollar" : n === "blockDollar" ? n : n === "inlineDollar" ? "none" : "inlineDollar";
var h2 = (n, r2) => {
  let e = "none";
  for (let i = 0; i < n.length && i < r2; i += 1) {
    if (n[i] === "\\" && n[i + 1] === "$") {
      i += 1;
      continue;
    }
...
var m = (n, r2) => {
  for (let e = r2 - 1; e >= 0; e -= 1) {
    if (n[e] === ")") return false;
    if (n[e] === "(") return e > 0 && n[e - 1] === "]" ? Bn(n, r2) : false;
    if (n[e] === `
`) return false;
  }
  return false;
...
var p = (n, r2, e) => {
  let i = 0;
  for (let l = r2 - 1; l >= 0; l -= 1) if (n[l] === `
`) {
    i = l + 1;
    break;
  }
  let o = n.length;
  for (let l = r2; l < n.length; l += 1) if (n[l] === `
`) {
    o = l;
    break;
  }
  let s2 = n.substring(i, o), t = 0, a = false;
  for (let l of s2) if (l === e) t += 1;
  else if (l !== " " && l !== "	") {
    a = true;
    break;
  }
...
var Sn = (n, r2, e, i) => e === "\\" || Z(n) && h2(n, r2) ? true : e !== "*" && i === "*" ? (r2 < n.length - 2 ? n[r2 + 2] : "") !== "*" : e === "*" || (!e || e === " " || e === "	" || e === `
`) && (!i || i === " " || i === "	" || i === `
`);
var k = (n) => n === " " || n === "	" || n === `
`;
var On = (n, r2) => !!(n && r2 && f(n) && f(r2));
var _n = (n, r2, e, i) => {
  let o = On(n, r2), s2 = !!r2 && !k(r2), t = !!n && !k(n);
  return o && e % 2 === 0 && !i ? { count: false } : t && e % 2 === 1 || s2 ? { count: true, inWordAsteriskChain: o } : { count: false };
};
var v = (n) => {
  let r2 = 0, e = false, i = false, o = n.length;
  for (let s2 = 0; s2 < o; s2 += 1) {
    if (n[s2] === "`" && s2 + 2 < o && n[s2 + 1] === "`" && n[s2 + 2] === "`") {
      e = !e, s2 += 2;
      continue;
    }
    if (e) continue;
    ...
    let t = s2 > 0 ? n[s2 - 1] : "", a = s2 < o - 1 ? n[s2 + 1] : "";
    if (Sn(n, s2, t, a)) continue;
    let l = _n(t, a, r2, i);
    l.count && (r2 += 1, i = l.inWordAsteriskChain);
  }
  return r2;
};
var Mn = (n, r2, e, i) => !!(e === "\\" || Z(n) && h2(n, r2) || m(n, r2) || j(n, r2) || e === "_" || i === "_" || e && i && f(e) && f(i));
var Pn = (n) => {
  let r2 = 0, e = false, i = n.length;
  for (let o = 0; o < i; o += 1) {
    ...
    if (e || n[o] !== "_") continue;
    let s2 = o > 0 ? n[o - 1] : "", t = o < i - 1 ? n[o + 1] : "";
    Mn(n, o, s2, t) || (r2 += 1);
  }
  return r2;
};
var $n = (n) => {
  let r2 = 0, e = 0, i = false;
  for (let o = 0; o < n.length; o += 1) {
    if (n[o] === "`" && o + 2 < n.length && n[o + 1] === "`" && n[o + 2] === "`") {
      e >= 3 && (r2 += Math.floor(e / 3)), e = 0, i = !i, o += 2;
      continue;
    }
    i || (n[o] === "*" ? e += 1 : (e >= 3 && (r2 += Math.floor(e / 3)), e = 0));
  }
...
var L = (n) => {
  ...
  for (let i = 0; i < n.length; i += 1) {
    ...
    e || n[i] === "*" && i + 1 < n.length && n[i + 1] === "*" && (r2 += 1, i += 1);
  }
  return r2;
};
var Q = (n) => {
  ...
  for (let i = 0; i < n.length; i += 1) {
    ...
    e || n[i] === "_" && i + 1 < n.length && n[i + 1] === "_" && (r2 += 1, i += 1);
  }
  return r2;
};
var En = (n, r2, e) => {
  if (!r2 || g.test(r2)) return true;
  let o = n.substring(0, e).lastIndexOf(`
`), s2 = o === -1 ? 0 : o + 1, t = n.substring(s2, e);
  return C.test(t) && r2.includes(`
`) ? true : p(n, e, "*");
};
var q = (n) => {
  let r2 = n.match(N);
  if (!r2) return n;
  let e = r2[2], i = n.lastIndexOf(r2[1]);
...
var Nn = (n) => {
  ...
  for (let e = 0; e < n.length; e += 1) {
    ...
    if (!r2 && n[e] === "*" && n[e - 1] !== "*" && n[e + 1] !== "*" && n[e - 1] !== "\\" && !h2(n, e)) {
      let i = e > 0 ? n[e - 1] : "", o = e < n.length - 1 ? n[e + 1] : "", s2 = !i || k(i), t = !o || k(o);
      if (s2 && t || i && o && f(i) && f(o) || t) continue;
      return e;
    }
  }
  return -1;
...
var x = (n) => {
  ...
  for (let e = 0; e < n.length; e += 1) {
    ...
    if (!r2 && n[e] === "_" && n[e - 1] !== "_" && n[e + 1] !== "_" && n[e - 1] !== "\\" && !h2(n, e) && !m(n, e)) {
...
`; ) r2 -= 1;
  if (r2 < n.length) {
    let e = n.slice(0, r2), i = n.slice(r2);
    return `${e}_${i}`;
  }
  return `${n}_`;
};
var Un = (n) => {
  ...
  let r2 = n.slice(0, -2);
  if (L(r2) % 2 !== 1) return null;
  let i = r2.indexOf("**"), o = x(r2);
  return i !== -1 && o !== -1 && i < o ? `${r2}_**` : null;
};
var nn = (n) => {
...
var Dn = (n, r2, e) => !r2 || g.test(r2) || c(n, e) || d(n, e) ? true : p(n, e, "*");
var en = (n) => {
...
var rn = (n) => {
  let r2 = n.match(Kn);
  return !r2 || r2.index === void 0 || c(n, r2.index) ? n : n.substring(0, r2.index).trimEnd();
};
...
var sn = (n, r2) => r2 >= 2 && n.substring(r2 - 2, r2 + 1) === "```" || r2 >= 1 && n.substring(r2 - 1, r2 + 2) === "```" || r2 <= n.length - 3 && n.substring(r2, r2 + 3) === "```";
var Gn = (n) => {
  let r2 = 0, e = false;
  for (let i = 0; i < n.length - 1; i += 1) n[i] === "`" && !sn(n, i) && (e = !e), !e && n[i] === "$" && n[i + 1] === "$" && (r2 += 1, i += 1);
  return r2;
};
var Fn = (n) => {
  let r2 = 0, e = false;
  for (let i = 0; i < n.length; i += 1) {
    ...
    !e && n[i] === "$" && (i + 1 < n.length && n[i + 1] === "$" ? i += 1 : r2 += 1);
  }
  return r2;
};
var Xn = (n) => {
...
var zn = (n, r2, e) => {
  ...
  let s2 = o > 0 && n[o - 1] === "!", t = s2 ? o - 1 : o, a = n.substring(0, t);
  if (s2) return a;
  let l = n.substring(o + 1, r2);
  return e === "text-only" ? `${a}${l}` : `${a}[${l}](streamdown:incomplete-link)`;
};
var an = (n, r2) => {
  for (let e = 0; e < r2; e++) if (n[e] === "[" && !c(n, e)) {
...
`);
  if (r2 === -1) return n;
  let e = n.substring(r2 + 1), i = n.substring(0, r2), o = e.trim();
...
var un = (n) => !n || typeof n != "string" || !n.includes("~") ? n : n.replace(qn, (r2, e, i) => {
  let o = i + e.length;
  return c(n, o) ? r2 : `${e}\\~`;
});
var fn = (n) => {
  ...
  if (r2) {
    ...
    let s2 = n.lastIndexOf(r2[1]);
    if (c(n, s2) || d(n, s2)) return n;
    if (((e = n.match(A)) == null ? void 0 : e.length) % 2 === 1) return `${n}~~`;
  } else {
    let o = n.match(z);
    if (o) {
      let s2 = n.lastIndexOf(o[0].slice(0, 2));
      if (c(n, s2) || d(n, s2)) return n;
...
var Vn = [{ handler: { name: "singleTilde", handle: un, priority: u.SINGLE_TILDE }, optionKey: "singleTilde" }, { handler: { name: "comparisonOperators", handle: y, priority: u.COMPARISON_OPERATORS }, optionKey: "comparisonOperators" }, { handler: { name: "htmlTags", handle: rn, priority: u.HTML_TAGS }, optionKey: "htmlTags" }, { handler: { name: "setextHeadings", handle: cn, priority: u.SETEXT_HEADINGS }, optionKey: "setextHeadings" }, { handler: { name: "links", handle: B, priority: u.LINKS }, optionKey: "links", earlyReturn: (n) => n.endsWith("](streamdown:incomplete-link)") }, { handler: { name: "boldItalic", handle: en, priority: u.BOLD_ITALIC }, optionKey: "boldItalic" }, { handler: { name: "bold", handle: q, priority: u.BOLD }, optionKey: "bold" }, { handler: { name: "italicDoubleUnderscore", handle: J, priority: u.ITALIC_DOUBLE_UNDERSCORE }, optionKey: "italic" }, { handler: { name: "italicSingleAsterisk", handle: V, priority: u.ITALIC_SINGLE_ASTERISK }, optionKey: "italic" }, { handler: { name: "italicSingleUnderscore", handle: nn, priority: u.ITALIC_SINGLE_UNDERSCORE }, optionKey: "italic" }, { handler: { name: "inlineCode", handle: on, priority: u.INLINE_CODE }, optionKey: "inlineCode" }, { handler: { name: "strikethrough", handle: fn, priority: u.STRIKETHROUGH }, optionKey: "strikethrough" }, { handler: { name: "katex", handle: tn, priority: u.KATEX }, optionKey: "katex" }, { handler: { name: "inlineKatex", handle: ln, priority: u.INLINE_KATEX }, optionKey: "inlineKatex" }];
var xn = (n) => {
  var e;
  let r2 = (e = n == null ? void 0 : n.linkMode) != null ? e : "protocol";
  return Vn.filter(({ handler: i, optionKey: o }) => i.name === "links" ? S(n == null ? void 0 : n.links) || S(n == null ? void 0 : n.images) : i.name === "inlineKatex" ? Jn(n == null ? void 0 : n.inlineKatex) : S(n == null ? void 0 : n[o])).map(({ handler: i, earlyReturn: o }) => i.name === "links" ? { handler: b(I({}, i), { handle: (s2) => B(s2, r2) }), earlyReturn: r2 === "protocol" ? o : void 0 } : { handler: i, earlyReturn: o });
};
var ne = (n, r2) => {
  var t;
  if (!n || typeof n != "string") return n;
  let e = n.endsWith(" ") && !n.endsWith("  ") ? n.slice(0, -1) : n, i = xn(r2), o = ((t = r2 == null ? void 0 : r2.handlers) != null ? t : []).map((a) => {
    var l;
    return { handler: b(I({}, a), { priority: (l = a.priority) != null ? l : u.DEFAULT }), earlyReturn: void 0 };
  }), s2 = [...i, ...o].sort((a, l) => {
    var O2, _2;
    return ((O2 = a.handler.priority) != null ? O2 : 0) - ((_2 = l.handler.priority) != null ? _2 : 0);
  });
  for (let { handler: a, earlyReturn: l } of s2) if (e = a.handle(e), l != null && l(e)) return e;
  return e;
};
var We = ne;
...
// node_modules/streamdown/node_modules/tailwind-merge/dist/bundle-mjs.mjs
...
function productionCreate(_2, jsx2, jsxs2) {
  ...
  function create2(_3, type, props, key) {
    ...
    const fn3 = isStaticChildren ? jsxs2 : jsx2;
...
function findComponentFromName(state, name2, allowExpression) {
  ...
  if (result.type === "Literal") {
    ...
    return own5.call(state.components, name3) ? state.components[name3] : name3;
  }
  if (state.evaluater) {
...
function transformStyleToCssCasing(from) {
  let to2 = from.replace(cap2, toDash);
...
var Processor = class _Processor extends CallableInstance {
  ...
  data(key, value) {
    if (typeof key === "string") {
      ...
      return own8.call(this.namespace, key) && this.namespace[key] || void 0;
    }
    if (key) {
      assertUnfrozen("data", this.frozen);
      this.namespace = key;
...
var _ = { exec: () => null };
function k2(u4, e = "") {
  let t = typeof u4 == "string" ? u4 : u4.source, n = { replace: (r2, i) => {
    let s2 = typeof i == "string" ? i : i.source;
    return s2 = s2.replace(m2.caret, "$1"), t = t.replace(r2, s2), n;
...
var K2 = { blockquote: Le, code: Oe, def: Se, fences: Te, heading: we, hr: C2, html: _e, lheading: ie, list: $e, newline: Re, paragraph: oe, table: _, text: Pe };
var ne2 = k2("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr", C2).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("blockquote", " {0,3}>").replace("code", "(?: {4}| {0,3}	)[^\\n]").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", v2).getRegex();
var Me = { ...K2, lheading: ye, table: ne2, paragraph: k2(j2).replace("hr", C2).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ne2).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", v2).getRegex() };
...
var We2 = k2(U2).replace("(?:-->|$)", "-->").getRegex();
var Xe = k2("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", We2).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex();
var q2 = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+(?!`)[^`]*?`+(?!`)|``+(?=\])|[^\[\]\\`])*?/;
...
var re2 = /[hH][tT][tT][pP][sS]?|[fF][tT][pP]/;
...
var N2 = { ...X2, emStrongRDelimAst: Ge, emStrongLDelim: He, delLDelim: Qe, delRDelim: Fe, url: k2(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", re2).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(), _backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/, del: /^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/, text: k2(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol", re2).getRegex() };
...
function V2(u4, e) {
  let t = u4.replace(m2.findPipe, (i, s2, a) => {
    ...
    for (; --l >= 0 && a[l] === "\\"; ) o = !o;
    return o ? "|" : " |";
  }), n = t.split(m2.splitPipe), r2 = 0;
  if (n[0].trim() || n.shift(), n.length > 0 && !n.at(-1)?.trim() && n.pop(), e) if (n.length > e) n.splice(e);
  else for (; n.length < e; ) n.push("");
  for (; r2 < n.length; r2++) n[r2] = n[r2].trim().replace(m2.slashPipe, "|");
  return n;
}
function I2(u4, e, t) {
  let n = u4.length;
  ...
  let r2 = 0;
  for (; r2 < n; ) {
    let i = u4.charAt(n - r2 - 1);
    if (i === e && !t) r2++;
    else if (i !== e && t) r2++;
    else break;
  }
  return u4.slice(0, n - r2);
}
function de(u4, e) {
...
function fe(u4, e, t, n, r2) {
  let i = e.href, s2 = e.title || null, a = u4[1].replace(r2.other.outputLinkReplace, "$1");
  n.state.inLink = true;
...
`) };
  ...
  fences(e) {
    ...
    if (t) {
      let n = t[0], r2 = nt(n, t[3] || "", this.rules);
...
`), S3 = this.blockquote(f2);
          s2[s2.length - 1] = S3, r2 = r2.substring(0, r2.length - R2.raw.length) + S3.raw, i = i.substring(0, i.length - R2.text.length) + S3.text;
          break;
        } else if (h3?.type === "list") {
          let R2 = h3, f2 = R2.raw + `
` + n.join(`
`), S3 = this.list(f2);
          s2[s2.length - 1] = S3, r2 = r2.substring(0, r2.length - h3.raw.length) + S3.raw, i = i.substring(0, i.length - R2.raw.length) + S3.raw, n = f2.substring(s2.at(-1).raw.length).split(`
`);
          continue;
        }
      }
...
`, 1)[0], R2 = !d2.trim(), f2 = 0;
        if (this.options.pedantic ? (f2 = 2, c2 = d2.trimStart()) : R2 ? f2 = t[1].length + 1 : (f2 = d2.search(this.rules.other.nonSpaceChar), f2 = f2 > 4 ? 1 : f2, c2 = d2.slice(f2), f2 += t[1].length), R2 && this.rules.other.blankLine.test(h3) && (p2 += h3 + `
`, e = e.substring(h3.length + 1), l = true), !l) {
          let S3 = this.rules.other.nextBulletRegex(f2), Y2 = this.rules.other.hrRegex(f2), ee = this.rules.other.fencesBeginRegex(f2), te = this.rules.other.headingBeginRegex(f2), me = this.rules.other.htmlBeginRegex(f2), xe = this.rules.other.blockquoteBeginRegex(f2);
          for (; e; ) {
            let Z2 = e.split(`
`, 1)[0], A2;
            if (h3 = Z2, this.options.pedantic ? (h3 = h3.replace(this.rules.other.listReplaceNesting, "  "), A2 = h3) : A2 = h3.replace(this.rules.other.tabCharGlobal, "    "), ee.test(h3) || te.test(h3) || me.test(h3) || xe.test(h3) || S3.test(h3) || Y2.test(h3)) break;
...
`, e = e.substring(Z2.length + 1), d2 = A2.slice(f2);
  ...
  table(e) {
    ...
    let n = V2(t[1]), r2 = t[2].replace(this.rules.other.tableAlignChars, "").split("|"), i = t[3]?.trim() ? t[3].replace(this.rules.other.tableRowBlankLine, "").split(`
...
` ? t[1].slice(0, -1) : t[1];
  ...
  emStrong(e, t, n = "") {
    ...
    if (!r2 || !r2[1] && !r2[2] && !r2[3] && !r2[4] || r2[4] && n.match(this.rules.other.unicodeAlphaNumeric)) return;
    if (!(r2[1] || r2[3] || "") || !n || this.rules.inline.punctuation.exec(n)) {
      let s2 = [...r2[0]].length - 1, a, o, l = s2, p2 = 0, c2 = r2[0][0] === "*" ? this.rules.inline.emStrongRDelimAst : this.rules.inline.emStrongRDelimUnd;
      for (c2.lastIndex = 0, t = t.slice(-1 * e.length + s2); (r2 = c2.exec(t)) !== null; ) {
        ...
        if (l -= o, l > 0) continue;
        o = Math.min(o, o + l + p2);
        let d2 = [...r2[0]][0].length, h3 = e.slice(0, s2 + r2.index + d2 + o);
        if (Math.min(s2, o) % 2) {
          let f2 = h3.slice(1, -1);
  ...
  del(e, t, n = "") {
    let r2 = this.rules.inline.delLDelim.exec(e);
    if (!r2) return;
    if (!(r2[1] || "") || !n || this.rules.inline.punctuation.exec(n)) {
      let s2 = [...r2[0]].length - 1, a, o, l = s2, p2 = this.rules.inline.delRDelim;
      for (p2.lastIndex = 0, t = t.slice(-1 * e.length + s2); (r2 = p2.exec(t)) !== null; ) {
        if (a = r2[1] || r2[2] || r2[3] || r2[4] || r2[5] || r2[6], !a || (o = [...a].length, o !== s2)) continue;
        if (r2[3] || r2[4]) {
          l += o;
          continue;
        }
        if (l -= o, l > 0) continue;
        o = Math.min(o, o + l);
        let c2 = [...r2[0]][0].length, d2 = e.slice(0, s2 + r2.index + c2 + o), h3 = d2.slice(s2, -s2);
...
var x2 = class u2 {
  ...
  tokenizer;
  constructor(e) {
    this.tokens = [], this.tokens.links = /* @__PURE__ */ Object.create(null), this.options = e || O, this.options.tokenizer = this.options.tokenizer || new w2(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = { inLink: false, inRawBlock: false, top: true };
...
var y2 = class {
  ...
  code({ text: e, lang: t, escaped: n }) {
    let r2 = (t || "").match(m2.notSpaceStart)?.[0], i = e.replace(m2.endingNewline, "") + `
`;
    return r2 ? '<pre><code class="language-' + T2(r2) + '">' + (n ? i : T2(i, true)) + `</code></pre>
...
`;
  ...
  link({ href: e, title: t, tokens: n }) {
    let r2 = this.parser.parseInline(n), i = J2(e);
    if (i === null) return r2;
    e = i;
    let s2 = '<a href="' + e + '"';
    return t && (s2 += ' title="' + T2(t) + '"'), s2 += ">" + r2 + "</a>", s2;
  }
...
var b2 = class u3 {
  ...
  textRenderer;
  constructor(e) {
    this.options = e || O, this.options.renderer = this.options.renderer || new y2(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new $3();
  }
  static parse(e, t) {
  ...
  parse(e) {
    ...
    for (let n = 0; n < e.length; n++) {
      ...
      if (this.options.extensions?.renderers?.[r2.type]) {
        let s2 = r2, a = this.options.extensions.renderers[s2.type].call({ parser: this }, s2);
...
Please report this to https://github.com/markedjs/marked.`, e) {
        let r2 = "<p>An error occurred:</p><pre>" + T2(n.message + "", true) + "</pre>";
...
var mr = 500;
function Ut2(e = {}) {
  let { immediate: t = false, debounceDelay: o = cr, rootMargin: n = dr, idleTimeout: r2 = mr } = e, [s2, a] = useState(false), l = useRef(null), i = useRef(null), c2 = useRef(null), d2 = useMemo(() => (u4) => {
    let f2 = Date.now();
    return window.setTimeout(() => {
      u4({ didTimeout: false, timeRemaining: () => Math.max(0, 50 - (Date.now() - f2)) });
    }, 1);
  }, []), p2 = useMemo(() => typeof window != "undefined" && window.requestIdleCallback ? (u4, f2) => window.requestIdleCallback(u4, f2) : d2, [d2]), m3 = useMemo(() => typeof window != "undefined" && window.cancelIdleCallback ? (u4) => window.cancelIdleCallback(u4) : (u4) => {
    clearTimeout(u4);
  }, []);
  return useEffect(() => {
    if (t) {
    ...
    let u4 = l.current;
    if (!u4) return;
    i.current && (clearTimeout(i.current), i.current = null), c2.current && (m3(c2.current), c2.current = null);
    let f2 = () => {
      i.current && (clearTimeout(i.current), i.current = null), c2.current && (m3(c2.current), c2.current = null);
    }, b3 = (g3) => {
      c2.current = p2((x3) => {
        x3.timeRemaining() > 0 || x3.didTimeout ? (a(true), g3.disconnect()) : c2.current = p2(() => {
          a(true), g3.disconnect();
        }, { timeout: r2 / 2 });
      }, { timeout: r2 });
    }, w3 = (g3) => {
      f2(), i.current = window.setTimeout(() => {
        var N3, T3;
        let x3 = g3.takeRecords();
        (x3.length === 0 || (T3 = (N3 = x3.at(-1)) == null ? void 0 : N3.isIntersecting) != null && T3) && b3(g3);
      }, o);
    }, y3 = (g3, x3) => {
      g3.isIntersecting ? w3(x3) : f2();
    }, h3 = new IntersectionObserver((g3) => {
      for (let x3 of g3) y3(x3, h3);
    }, { rootMargin: n, threshold: 0 });
    return h3.observe(u4), () => {
      i.current && clearTimeout(i.current), c2.current && m3(c2.current), h3.disconnect();
    };
  }, [t, o, n, r2, m3, p2]), { shouldRender: s2, containerRef: l };
}
var ur = 320;
var pr = 4;
...
function pt(e) {
  var s2, a;
  let t = (s2 = e == null ? void 0 : e.now) != null ? s2 : eo, o = (a = e == null ? void 0 : e.maxBacklogMs) != null ? a : ur, n = 0, r2 = 0;
  return { now: t, beginPass(l) {
    r2 = Math.min(Math.max(n, l), l + o);
  }, mark() {
    return r2;
  }, rewind(l) {
    r2 = l;
  }, take(l, i, c2) {
    if (l <= 0) return { baseDelay: 0, step: Math.max(0, i) };
    let d2 = Math.max(0, i), p2 = d2 === 0 ? 0 : Math.min(d2, pr), m3 = Math.max(r2, c2), u4 = c2 + o, f2 = m3 + Math.max(0, l - 1) * d2, b3 = d2;
    if (f2 > u4 && l > 1) if (m3 < u4) {
      let y3 = u4 - m3;
      b3 = Math.max(p2, y3 / (l - 1));
    } else b3 = p2;
    let w3 = Math.max(0, Math.round(m3 - c2));
    return r2 = m3 + l * b3, { baseDelay: w3, step: b3 };
  }, commitPass() {
    n = r2;
  } };
}
var to = /\s/;
var ft = /^\s+$/;
var fr = /* @__PURE__ */ new Set(["pre", "svg", "math", "annotation"]);
var gr = /* @__PURE__ */ new Set(["img", "hr"]);
var Te2 = (e) => typeof e == "object" && e !== null && "type" in e && e.type === "element";
var gt = (e) => e.some((t) => Te2(t) && fr.has(t.tagName));
var br = (e) => {
  for (let t = e.length - 1; t >= 0; t--) {
    let o = e[t];
    if (Te2(o) && o.tagName === "li") return o;
  }
};
var hr = (e, t, o, n) => {
  e.properties != null || (e.properties = {}), e.properties["data-sd-animate-marker"] = true;
  let r2 = typeof e.properties.style == "string" ? `${e.properties.style};` : "";
  e.properties.style = `${r2}--sd-marker-duration:${t}ms;--sd-marker-delay:${Math.round(o)}ms;--sd-marker-easing:${n}`;
};
var yr = /* @__PURE__ */ new Set(["ul", "ol", "li"]);
var oo = (e) => {
  for (let t of e.children) if (Te2(t)) {
...
var no = (e, t, o, n) => {
  e.properties != null || (e.properties = {}), e.properties["data-sd-animate"] = true;
  let r2 = typeof e.properties.style == "string" ? `${e.properties.style};` : "";
  e.properties.style = `${r2}--sd-animation:sd-${t.animation};--sd-duration:${o}ms;--sd-easing:${t.easing};--sd-delay:${Math.round(n)}ms`;
};
var wr = (e, t, o, n) => {
  let r2 = oo(e);
  r2 && no(r2, t, o, n);
};
var Cr = (e, t, o, n, r2, s2) => {
  if (gt(t)) return;
  let a = n.prevContentLength, l = r2.count;
  r2.count += 1;
  let i = a > 0 && l < a, c2 = i ? 0 : s2.baseDelay + r2.newIndex++ * s2.step;
  no(e, o, i ? 0 : o.duration, c2);
};
var xr = (e) => {
  var t;
  for (let o = e.length - 1; o >= 0; o -= 1) {
    ...
    if (Te2(n)) {
      ...
      n.properties = { ...n.properties, "data-sd-animated": true };
    }
  }
};
var ro = (e) => {
...
var kr = (e, t, o, n, r2, s2) => {
  let a = `--sd-animation:sd-${t};--sd-duration:${r2 ? 0 : o}ms;--sd-easing:${n}`;
  return s2 && (a += `;--sd-delay:${Math.round(s2)}ms`), { type: "element", tagName: "span", properties: { "data-sd-animate": true, style: a }, children: [{ type: "text", value: e }] };
};
var Gt = (e, t) => !(e > 0 && t < e);
var ut = (e) => Te2(e) && gr.has(e.tagName);
var vr = (e, t, o) => {
  let n = 0, r2 = 0;
  return visitParents(e, (s2) => s2.type === "text" || ut(s2), (s2, a) => {
    if (gt(a)) return SKIP;
    if (ut(s2)) {
      Gt(o, r2) && (n += 1), r2 += 1;
      return;
    ...
    for (let c2 of i) {
      let d2 = r2;
      r2 += c2.length, !ft.test(c2) && Gt(o, d2) && (n += 1);
    }
  }), n;
};
var Tr = (e, t, o, n, r2, s2) => {
  var y3;
  let a = t.at(-1);
  ...
  let d2 = o.sep === "char" ? so(c2) : ro(c2), p2 = n.prevContentLength, m3 = false, u4 = br(t), f2 = !!(u4 && !((y3 = u4.properties) != null && y3["data-sd-animate-marker"])), b3 = false, w3 = d2.map((h3) => {
    let g3 = r2.count;
    if (r2.count += h3.length, ft.test(h3)) return { type: "text", value: h3 };
    let x3 = p2 > 0 && g3 < p2, I3 = x3 ? 0 : s2.baseDelay + r2.newIndex++ * s2.step;
    if (m3 = true, u4 && f2 && !b3) {
      let N3 = x3 ? 0 : o.duration;
      hr(u4, N3, I3, o.easing), wr(u4, o, N3, I3), b3 = true;
    }
    return kr(h3, o.animation, o.duration, o.easing, x3, I3);
  });
  return m3 && xr(t), l.children.splice(i, 1, ...w3), i + w3.length;
};
var Pr = 0;
function je2(e) {
  var s2, a, l, i, c2;
  let t = { animation: (s2 = e == null ? void 0 : e.animation) != null ? s2 : "fadeIn", duration: (a = e == null ? void 0 : e.duration) != null ? a : 150, easing: (l = e == null ? void 0 : e.easing) != null ? l : "ease", sep: (i = e == null ? void 0 : e.sep) != null ? i : "word", stagger: (c2 = e == null ? void 0 : e.stagger) != null ? c2 : 40, timeline: e == null ? void 0 : e.timeline }, o = { committedCharCount: 0, prevContentLength: 0, lastRenderCharCount: 0, pendingMark: null }, n = Pr++, r2 = () => (d2) => {
    var b3;
    let p2 = { count: 0, newIndex: 0 };
    o.prevContentLength = o.committedCharCount;
    let m3 = t.timeline, u4 = (b3 = m3 == null ? void 0 : m3.now()) != null ? b3 : eo();
    m3 && (o.pendingMark === null ? o.pendingMark = m3.mark() : m3.rewind(o.pendingMark));
    let f2 = m3 ? m3.take(vr(d2, t, o.prevContentLength), t.stagger, u4) : { baseDelay: 0, step: t.stagger };
    visitParents(d2, (w3) => w3.type === "text" || ut(w3), (w3, y3) => {
      if (w3.type === "text") return Tr(w3, y3, t, o, p2, f2);
      Cr(w3, y3, t, o, p2, f2);
    }), o.lastRenderCharCount = p2.count, o.prevContentLength = 0;
  };
  return Object.defineProperty(r2, "name", { value: `rehypeAnimate$${n}` }), { name: "animate", type: "animate", rehypePlugin: r2, setPrevContentLength(d2) {
    o.committedCharCount = d2, o.prevContentLength = d2;
...
je2();
var bt = createContext(false);
var ht = () => useContext(bt);
var fe2 = (...e) => twMerge(clsx(e));
var Nr = (e, t) => {
  if (!e || !t) return t;
  let o = `${e}:`;
  return t.split(/\s+/).filter(Boolean).map((n) => n.startsWith(o) ? n : `${e}:${n}`).join(" ");
};
var lo = (e) => e ? (...t) => Nr(e, twMerge(clsx(t))) : fe2;
var J3 = (e, t, o) => {
  let n = typeof t == "string" && o.startsWith("text/csv") ? "\uFEFF" : "", r2 = typeof t == "string" ? new Blob([n + t], { type: o }) : t, s2 = URL.createObjectURL(r2), a = document.createElement("a");
  a.href = s2, a.download = e, document.body.appendChild(a), a.click(), document.body.removeChild(a), URL.revokeObjectURL(s2);
};
var _e2 = createContext(fe2);
var C3 = () => useContext(_e2);
var Er = 8;
var ze2 = (e) => {
...
var $e2 = (e, t, o) => {
  let n = useRef(null), r2 = useRef(true), s2 = useRef(false);
  return useEffect(() => {
    let a = n.current;
    if (!(a && t)) return;
    let l = () => {
    ...
    e && !s2.current && (r2.current = true), e || (r2.current = true), s2.current = e;
  }, [e]), useEffect(() => {
    let a = n.current;
    a && t && e && r2.current && a.scrollTo({ top: a.scrollHeight, behavior: "instant" });
  }, [e, t, o]), n;
};
var Ar = fe2("block");
var Hr = fe2("block", "before:content-[counter(line)]", "before:inline-block", "before:[counter-increment:line]", "before:w-6", "before:mr-4", "before:text-[13px]", "before:text-right", "before:text-muted-foreground/50", "before:font-mono", "before:select-none");
var Br = (e) => {
  let t = {};
  for (let o of e.split(";")) {
    let n = o.indexOf(":");
    if (n > 0) {
      let r2 = o.slice(0, n).trim(), s2 = o.slice(n + 1).trim();
      r2 && s2 && (t[r2] = s2);
    }
  }
  return t;
};
var co = memo(({ children: e, result: t, language: o, className: n, maxHeight: r2, startLine: s2, lineNumbers: a = true, ...l }) => {
  let i = C3(), { isAnimating: c2 } = useContext(S2), d2 = ze2(r2), p2 = $e2(c2, !!d2, t), m3 = useMemo(() => i(Hr), [i]), u4 = useMemo(() => i(Ar), [i]), f2 = useMemo(() => {
    let b3 = {};
    return t.bg && (b3["--sdm-bg"] = t.bg), t.fg && (b3["--sdm-fg"] = t.fg), t.rootStyle && Object.assign(b3, Br(t.rootStyle)), b3;
  }, [t.bg, t.fg, t.rootStyle]);
  return jsx("div", { className: i(n, d2 ? "overflow-y-auto" : null, "overflow-x-auto rounded-md border border-border bg-background p-4 text-sm"), "data-language": o, "data-streamdown": "code-block-body", ref: p2, style: d2 ? { maxHeight: d2 } : void 0, ...l, children: jsx("pre", { className: i(n, "bg-[var(--sdm-bg,inherit)]", "dark:bg-[var(--shiki-dark-bg,var(--sdm-bg,inherit))]"), style: f2, children: jsx("code", { className: a ? i("[counter-increment:line_0] [counter-reset:line]") : void 0, style: a && s2 && s2 > 1 ? { counterReset: `line ${s2 - 1}` } : void 0, children: t.tokens.map((b3, w3) => jsx("span", { className: a ? m3 : u4, children: b3.length === 0 || b3.length === 1 && b3[0].content === "" ? `
` : b3.map((y3, h3) => {
    let g3 = {}, x3 = !!y3.bgColor;
    if (y3.color && (g3["--sdm-c"] = y3.color), y3.bgColor && (g3["--sdm-tbg"] = y3.bgColor), y3.htmlStyle) for (let [I3, N3] of Object.entries(y3.htmlStyle)) I3 === "color" ? g3["--sdm-c"] = N3 : I3 === "background-color" ? (g3["--sdm-tbg"] = N3, x3 = true) : g3[I3] = N3;
    return jsx("span", { className: i("text-[var(--sdm-c,inherit)]", "dark:text-[var(--shiki-dark,var(--sdm-c,inherit))]", x3 && "bg-[var(--sdm-tbg)]", x3 && "dark:bg-[var(--shiki-dark-bg,var(--sdm-tbg))]"), style: g3, ...y3.htmlAttrs, children: y3.content }, h3);
  }) }, w3)) }) }) });
});
var xt = ({ className: e, language: t, style: o, isIncomplete: n, ...r2 }) => {
  let s2 = C3();
  return jsx("div", { className: s2("my-4 flex w-full flex-col gap-2 rounded-xl border border-border bg-sidebar p-2", e), "data-incomplete": n || void 0, "data-language": t, "data-streamdown": "code-block", style: { contentVisibility: "auto", containIntrinsicSize: "auto 200px", ...o }, ...r2 });
};
var kt = createContext({ code: "" });
var qe2 = () => useContext(kt);
var vt = ({ language: e }) => {
  let t = C3();
  return jsx("div", { className: t("flex h-8 items-center text-muted-foreground text-xs"), "data-language": e, "data-streamdown": "code-block-header", children: jsx("span", { className: t("ml-1 font-mono lowercase"), children: e }) });
};
var $r = (e) => {
  let t = e.length;
  for (; t > 0 && e[t - 1] === `
...
var qr = lazy(() => import("./highlighted-body-KPVGNVTW-NUJGR42L.js").then((e) => ({ default: e.HighlightedCodeBlockBody })));
...
var ie2 = (e, t, o) => {
  ...
  let r2 = n.download;
  return typeof r2 != "object" ? o : r2.filename || o;
};
var Po = { "1c": "1c", "1c-query": "1cq", abap: "abap", "actionscript-3": "as", ada: "ada", adoc: "adoc", "angular-html": "html", "angular-ts": "ts", apache: "conf", apex: "cls", apl: "apl", applescript: "applescript", ara: "ara", asciidoc: "adoc", asm: "asm", astro: "astro", awk: "awk", ballerina: "bal", bash: "sh", bat: "bat", batch: "bat", be: "be", beancount: "beancount", berry: "berry", bibtex: "bib", bicep: "bicep", blade: "blade.php", bsl: "bsl", c: "c", "c#": "cs", "c++": "cpp", cadence: "cdc", cairo: "cairo", cdc: "cdc", clarity: "clar", clj: "clj", clojure: "clj", "closure-templates": "soy", cmake: "cmake", cmd: "cmd", cobol: "cob", codeowners: "CODEOWNERS", codeql: "ql", coffee: "coffee", coffeescript: "coffee", "common-lisp": "lisp", console: "sh", coq: "v", cpp: "cpp", cql: "cql", crystal: "cr", cs: "cs", csharp: "cs", css: "css", csv: "csv", cue: "cue", cypher: "cql", d: "d", dart: "dart", dax: "dax", desktop: "desktop", diff: "diff", docker: "dockerfile", dockerfile: "dockerfile", dotenv: "env", "dream-maker": "dm", edge: "edge", elisp: "el", elixir: "ex", elm: "elm", "emacs-lisp": "el", erb: "erb", erl: "erl", erlang: "erl", f: "f", "f#": "fs", f03: "f03", f08: "f08", f18: "f18", f77: "f77", f90: "f90", f95: "f95", fennel: "fnl", fish: "fish", fluent: "ftl", for: "for", "fortran-fixed-form": "f", "fortran-free-form": "f90", fs: "fs", fsharp: "fs", fsl: "fsl", ftl: "ftl", gdresource: "tres", gdscript: "gd", gdshader: "gdshader", genie: "gs", gherkin: "feature", "git-commit": "gitcommit", "git-rebase": "gitrebase", gjs: "js", gleam: "gleam", "glimmer-js": "js", "glimmer-ts": "ts", glsl: "glsl", gnuplot: "plt", go: "go", gql: "gql", graphql: "graphql", groovy: "groovy", gts: "gts", hack: "hack", haml: "haml", handlebars: "hbs", haskell: "hs", haxe: "hx", hbs: "hbs", hcl: "hcl", hjson: "hjson", hlsl: "hlsl", hs: "hs", html: "html", "html-derivative": "html", http: "http", hxml: "hxml", hy: "hy", imba: "imba", ini: "ini", jade: "jade", java: "java", javascript: "js", jinja: "jinja", jison: "jison", jl: "jl", js: "js", json: "json", json5: "json5", jsonc: "jsonc", jsonl: "jsonl", jsonnet: "jsonnet", jssm: "jssm", jsx: "jsx", julia: "jl", kotlin: "kt", kql: "kql", kt: "kt", kts: "kts", kusto: "kql", latex: "tex", lean: "lean", lean4: "lean", less: "less", liquid: "liquid", lisp: "lisp", lit: "lit", llvm: "ll", log: "log", logo: "logo", lua: "lua", luau: "luau", make: "mak", makefile: "mak", markdown: "md", marko: "marko", matlab: "m", md: "md", mdc: "mdc", mdx: "mdx", mediawiki: "wiki", mermaid: "mmd", mips: "s", mipsasm: "s", mmd: "mmd", mojo: "mojo", move: "move", nar: "nar", narrat: "narrat", nextflow: "nf", nf: "nf", nginx: "conf", nim: "nim", nix: "nix", nu: "nu", nushell: "nu", objc: "m", "objective-c": "m", "objective-cpp": "mm", ocaml: "ml", pascal: "pas", perl: "pl", perl6: "p6", php: "php", plsql: "pls", po: "po", polar: "polar", postcss: "pcss", pot: "pot", potx: "potx", powerquery: "pq", powershell: "ps1", prisma: "prisma", prolog: "pl", properties: "properties", proto: "proto", protobuf: "proto", ps: "ps", ps1: "ps1", pug: "pug", puppet: "pp", purescript: "purs", py: "py", python: "py", ql: "ql", qml: "qml", qmldir: "qmldir", qss: "qss", r: "r", racket: "rkt", raku: "raku", razor: "cshtml", rb: "rb", reg: "reg", regex: "regex", regexp: "regexp", rel: "rel", riscv: "s", rs: "rs", rst: "rst", ruby: "rb", rust: "rs", sas: "sas", sass: "sass", scala: "scala", scheme: "scm", scss: "scss", sdbl: "sdbl", sh: "sh", shader: "shader", shaderlab: "shader", shell: "sh", shellscript: "sh", shellsession: "sh", smalltalk: "st", solidity: "sol", soy: "soy", sparql: "rq", spl: "spl", splunk: "spl", sql: "sql", "ssh-config": "config", stata: "do", styl: "styl", stylus: "styl", svelte: "svelte", swift: "swift", "system-verilog": "sv", systemd: "service", talon: "talon", talonscript: "talon", tasl: "tasl", tcl: "tcl", templ: "templ", terraform: "tf", tex: "tex", tf: "tf", tfvars: "tfvars", toml: "toml", ts: "ts", "ts-tags": "ts", tsp: "tsp", tsv: "tsv", tsx: "tsx", turtle: "ttl", twig: "twig", typ: "typ", typescript: "ts", typespec: "tsp", typst: "typ", v: "v", vala: "vala", vb: "vb", verilog: "v", vhdl: "vhdl", vim: "vim", viml: "vim", vimscript: "vim", vue: "vue", "vue-html": "html", "vue-vine": "vine", vy: "vy", vyper: "vy", wasm: "wasm", wenyan: "wy", wgsl: "wgsl", wiki: "wiki", wikitext: "wiki", wit: "wit", wl: "wl", wolfram: "wl", xml: "xml", xsl: "xsl", yaml: "yaml", yml: "yml", zenscript: "zs", zig: "zig", zsh: "zsh", \u6587\u8A00: "wy" };
var It = ({ onDownload: e, onError: t, language: o, children: n, className: r2, code: s2, ...a }) => {
  let l = C3(), { code: i } = qe2(), { isAnimating: c2, controls: d2 } = useContext(S2), p2 = B3(), m3 = H3(), u4 = s2 != null ? s2 : i, f2 = o && o in Po ? Po[o] : "txt", b3 = `${ie2(d2, "code", "file")}.${f2}`, w3 = "text/plain", y3 = () => {
    try {
      J3(b3, u4, w3), e == null || e();
    } catch (h3) {
      t == null || t(h3);
    }
  };
  return jsx("button", { "aria-label": p2.downloadFile, className: l("cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50", r2), "data-streamdown": "code-block-download-button", disabled: c2, onClick: y3, title: p2.downloadFile, type: "button", ...a, children: n != null ? n : jsx(m3.DownloadIcon, { size: 14 }) });
};
var Xe2 = () => {
  let { Loader2Icon: e } = H3(), t = C3();
  return jsxs("div", { className: t("w-full divide-y divide-border overflow-hidden rounded-xl border border-border"), children: [jsx("div", { className: t("h-[46px] w-full bg-muted/80") }), jsx("div", { className: t("flex w-full items-center justify-center p-4"), children: jsx(e, { className: t("size-4 animate-spin") }) })] });
};
var cs = /\.[^/.]+$/;
var So = ({ node: e, className: t, src: o, alt: n, onLoad: r2, onError: s2, showControls: a = true, showDownloadControl: l = true, ...i }) => {
  let { DownloadIcon: c2 } = H3(), d2 = C3(), p2 = useRef(null), [m3, u4] = useState(false), [f2, b3] = useState(false), w3 = B3(), y3 = i.width != null || i.height != null, g3 = (m3 || y3) && !f2 && a && l, x3 = f2 && !y3;
  useEffect(() => {
    let M3 = p2.current;
    if (M3 != null && M3.complete) {
      let E3 = M3.naturalWidth > 0;
      u4(E3), b3(!E3);
    }
  }, []);
  let I3 = useCallback((M3) => {
    u4(true), b3(false), r2 == null || r2(M3);
  }, [r2]), N3 = useCallback((M3) => {
    u4(false), b3(true), s2 == null || s2(M3);
  }, [s2]), T3 = async () => {
    if (o) try {
      let E3 = await (await fetch(o)).blob(), _2 = new URL(o, window.location.origin).pathname.split("/").pop() || "", W3 = _2.split(".").pop(), X3 = _2.includes(".") && W3 !== void 0 && W3.length <= 4, K3 = "";
      if (X3) K3 = _2;
      else {
        let j3 = E3.type, O2 = "png";
        j3.includes("jpeg") || j3.includes("jpg") ? O2 = "jpg" : j3.includes("png") ? O2 = "png" : j3.includes("svg") ? O2 = "svg" : j3.includes("gif") ? O2 = "gif" : j3.includes("webp") && (O2 = "webp"), K3 = `${(n || _2 || "image").replace(cs, "")}.${O2}`;
      }
      J3(K3, E3, E3.type);
    } catch (M3) {
  ...
  return o ? jsxs("div", { className: d2("group relative my-4 inline-block"), "data-streamdown": "image-wrapper", children: [jsx("img", { alt: n, className: d2("max-w-full rounded-lg", x3 && "hidden", t), "data-streamdown": "image", onError: N3, onLoad: I3, ref: p2, src: o, ...i }), x3 && jsx("span", { className: d2("text-muted-foreground text-xs italic"), "data-streamdown": "image-fallback", children: w3.imageNotAvailable }), a && jsx("div", { className: d2("pointer-events-none absolute inset-0 hidden rounded-lg bg-black/10 group-hover:block"), "data-streamdown": "image-overlay" }), g3 && jsx("button", { className: d2("absolute right-2 bottom-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border border-border bg-background/90 shadow-sm backdrop-blur-sm transition-all duration-200 hover:bg-background", "opacity-0 group-hover:opacity-100"), onClick: T3, title: w3.downloadImage, type: "button", children: jsx(c2, { size: 14 }) })] }) : null;
};
var Ne2 = 0;
var be2 = () => {
  Ne2 += 1, Ne2 === 1 && (document.body.style.overflow = "hidden");
};
var he2 = () => {
  Ne2 = Math.max(0, Ne2 - 1), Ne2 === 0 && (document.body.style.overflow = "");
};
var Lo = ({ url: e, isOpen: t, onClose: o, onConfirm: n }) => {
  let { CheckIcon: r2, CopyIcon: s2, ExternalLinkIcon: a, XIcon: l } = H3(), i = C3(), [c2, d2] = useState(false), p2 = B3(), m3 = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(e), d2(true), setTimeout(() => d2(false), 2e3);
    } catch (b3) {
    }
  }, [e]), u4 = useCallback(() => {
    n(), o();
  ...
  if (useEffect(() => {
    if (t) {
      ...
      return document.addEventListener("keydown", b3), () => {
        document.removeEventListener("keydown", b3), he2();
      };
    }
  }, [t, o]), !t || typeof document == "undefined") return null;
  let f2 = jsx("div", { className: i("fixed inset-0 z-50 flex items-center justify-center bg-background/50 backdrop-blur-sm"), "data-streamdown": "link-safety-modal", onClick: o, onKeyDown: (b3) => {
    b3.key === "Escape" && o();
  }, role: "button", tabIndex: 0, children: jsxs("div", { className: i("relative mx-4 flex w-full max-w-md flex-col gap-4 rounded-xl border bg-background p-6 shadow-lg"), onClick: (b3) => b3.stopPropagation(), onKeyDown: (b3) => b3.stopPropagation(), role: "presentation", children: [jsx("button", { className: i("absolute top-4 right-4 rounded-md p-1 text-muted-foreground transition-all hover:bg-muted hover:text-foreground"), onClick: o, title: p2.close, type: "button", children: jsx(l, { size: 16 }) }), jsxs("div", { className: i("flex flex-col gap-2"), children: [jsxs("div", { className: i("flex items-center gap-2 font-semibold text-lg"), children: [jsx(a, { size: 20 }), jsx("span", { children: p2.openExternalLink })] }), jsx("p", { className: i("text-muted-foreground text-sm"), children: p2.externalLinkWarning })] }), jsx("div", { className: i("break-all rounded-md bg-muted p-3 font-mono text-sm", e.length > 100 && "max-h-32 overflow-y-auto"), children: e }), jsxs("div", { className: i("flex gap-2"), children: [jsx("button", { className: i("flex flex-1 items-center justify-center gap-2 rounded-md border bg-background px-4 py-2 font-medium text-sm transition-all hover:bg-muted"), onClick: m3, type: "button", children: c2 ? jsxs(Fragment, { children: [jsx(r2, { size: 14 }), jsx("span", { children: p2.copied })] }) : jsxs(Fragment, { children: [jsx(s2, { size: 14 }), jsx("span", { children: p2.copyLink })] }) }), jsxs("button", { className: i("flex flex-1 items-center justify-center gap-2 rounded-md bg-primary px-4 py-2 font-medium text-primary-foreground text-sm transition-all hover:bg-primary/90"), onClick: u4, type: "button", children: [jsx(a, { size: 14 }), jsx("span", { children: p2.openLink })] })] })] }) });
  return createPortal(f2, document.body);
};
var Ke2 = createContext(null);
var St = () => useContext(Ke2);
var El = () => {
  var t;
  let e = St();
...
var Ho = (e, t) => {
  ...
  return new Promise((r2, s2) => {
    ...
    l.crossOrigin = "anonymous", l.onload = () => {
      let i = document.createElement("canvas"), c2 = l.width * o, d2 = l.height * o;
      i.width = c2, i.height = d2;
      let p2 = i.getContext("2d");
      if (!p2) {
        s2(new Error("Failed to create 2D canvas context for PNG export"));
        return;
      }
      p2.drawImage(l, 0, 0, c2, d2), i.toBlob((m3) => {
        if (!m3) {
...
var Je2 = ({ chart: e, children: t, className: o, onDownload: n, config: r2, onError: s2 }) => {
  let a = C3(), [l, i] = useState(false), c2 = useRef(null), { isAnimating: d2, controls: p2 } = useContext(S2), m3 = H3(), u4 = ye2(), f2 = B3(), b3 = ie2(p2, "mermaid", "diagram"), w3 = async (y3) => {
    try {
      if (y3 === "mmd") {
        let T3 = `${b3}.mmd`;
        J3(T3, e, "text/plain"), i(false), n == null || n(y3);
        return;
      }
      if (!u4) {
      ...
      let h3 = u4.getMermaid(r2), g3 = e.split("").reduce((T3, M3) => (T3 << 5) - T3 + M3.charCodeAt(0) | 0, 0), x3 = `mermaid-${Math.abs(g3)}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, { svg: I3 } = await h3.render(x3, e);
      if (!I3) {
      ...
      if (y3 === "svg") {
        let T3 = `${b3}.svg`;
        J3(T3, N3, "image/svg+xml"), i(false), n == null || n(y3);
        return;
      }
      if (y3 === "png") {
        let T3 = await Ho(N3);
        J3(`${b3}.png`, T3, "image/png"), n == null || n(y3), i(false);
        return;
      }
    } catch (h3) {
      s2 == null || s2(h3);
  ...
  return useEffect(() => {
    let y3 = (h3) => {
      let g3 = h3.composedPath();
      c2.current && !g3.includes(c2.current) && i(false);
    };
    return document.addEventListener("mousedown", y3), () => {
      document.removeEventListener("mousedown", y3);
    };
  }, []), jsxs("div", { className: a("relative"), ref: c2, children: [jsx("button", { "aria-label": f2.downloadDiagram, className: a("cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50", o), disabled: d2, onClick: () => i(!l), title: f2.downloadDiagram, type: "button", children: t != null ? t : jsx(m3.DownloadIcon, { "aria-hidden": "true", size: 14 }) }), l ? jsxs("div", { className: a("absolute top-full right-0 z-10 mt-1 min-w-[120px] overflow-hidden rounded-md border border-border bg-background shadow-lg"), children: [jsx("button", { "aria-label": f2.downloadDiagramAsSvg, className: a("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => w3("svg"), title: f2.downloadDiagramAsSvg, type: "button", children: f2.mermaidFormatSvg }), jsx("button", { "aria-label": f2.downloadDiagramAsPng, className: a("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => w3("png"), title: f2.downloadDiagramAsPng, type: "button", children: f2.mermaidFormatPng }), jsx("button", { "aria-label": f2.downloadDiagramAsMmd, className: a("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => w3("mmd"), title: f2.downloadDiagramAsMmd, type: "button", children: f2.mermaidFormatMmd })] }) : null] });
};
var Fo = ({ chart: e, config: t, onFullscreen: o, onExit: n, className: r2, ...s2 }) => {
  let { Maximize2Icon: a, XIcon: l } = H3(), i = C3(), [c2, d2] = useState(false), { isAnimating: p2, controls: m3 } = useContext(S2), u4 = B3(), f2 = (() => {
    if (typeof m3 == "boolean") return m3;
    let h3 = m3.mermaid;
    return h3 === false ? false : h3 === true || h3 === void 0 ? true : h3.panZoom !== false;
  })(), b3 = (() => {
    if (typeof m3 == "boolean") return m3;
    let h3 = m3.mermaid;
    return h3 === false ? false : h3 === true || h3 === void 0 ? true : h3.download !== false;
  })(), w3 = (() => {
    if (typeof m3 == "boolean") return m3;
    let h3 = m3.mermaid;
    return h3 === false ? false : h3 === true || h3 === void 0 ? true : h3.copy !== false;
  })(), y3 = () => {
    d2(!c2);
  };
  return useEffect(() => {
    if (c2) {
      be2();
      let h3 = (g3) => {
        g3.key === "Escape" && d2(false);
      };
      return document.addEventListener("keydown", h3), () => {
        document.removeEventListener("keydown", h3), he2();
      };
    }
  }, [c2]), useEffect(() => {
    c2 ? o == null || o() : n && n();
  }, [c2, o, n]), jsxs(Fragment, { children: [jsx("button", { className: i("cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50", r2), disabled: p2, onClick: y3, title: u4.viewFullscreen, type: "button", ...s2, "aria-label": u4.viewFullscreen, children: jsx(a, { "aria-hidden": "true", size: 14 }) }), c2 ? createPortal(jsxs("div", { "aria-label": u4.viewFullscreen, "aria-modal": "true", className: i("fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-sm"), "data-streamdown": "mermaid-fullscreen", onClick: y3, onKeyDown: (h3) => {
    h3.key === "Escape" && y3();
  }, role: "dialog", children: [jsxs("div", { className: i("absolute top-4 right-4 z-10 flex items-center gap-1"), onClick: (h3) => h3.stopPropagation(), onKeyDown: (h3) => h3.stopPropagation(), role: "presentation", children: [b3 ? jsx(Je2, { chart: e, config: t }) : null, w3 ? jsx(ge2, { code: e }) : null, jsx("button", { "aria-label": u4.exitFullscreen, className: i("rounded-md p-2 text-muted-foreground transition-all hover:bg-muted hover:text-foreground"), onClick: y3, title: u4.exitFullscreen, type: "button", children: jsx(l, { "aria-hidden": "true", size: 20 }) })] }), jsx("div", { className: i("flex size-full items-center justify-center p-4"), onClick: (h3) => h3.stopPropagation(), onKeyDown: (h3) => h3.stopPropagation(), role: "presentation", children: jsx(Vo, { chart: e, className: i("size-full [&_svg]:h-auto [&_svg]:w-auto"), config: t, fullscreen: true, showControls: f2 }) })] }), document.body) : null] });
};
function Et(e) {
  var o;
...
var we2 = (e) => {
  ...
  for (let s2 of n) t.push(Et(s2).trim());
  let r2 = e.querySelectorAll("tbody tr");
  for (let s2 of r2) {
    let a = [], l = s2.querySelectorAll("td");
...
var ce2 = (e, t = ",") => {
  ...
  let { headers: n, rows: r2 } = e, s2 = (c2) => {
    let d2 = false;
    for (let p2 of c2) if (p2 === o || p2 === '"' || p2 === `
` || p2 === "\r") {
      d2 = true;
      break;
    }
    return d2 ? `"${c2.replace(/"/g, '""')}"` : c2;
  }, a = n.length > 0 ? r2.length + 1 : r2.length, l = new Array(a), i = 0;
  n.length > 0 && (l[i] = n.map(s2).join(o), i += 1);
  for (let c2 of r2) l[i] = c2.map(s2).join(o), i += 1;
  return l.join(`
`);
};
var Lt = (e) => {
...
` || d2 === "\r") {
    ...
    if (!i) return l;
    let c2 = [];
    for (let d2 of l) d2 === "	" ? c2.push("\\t") : d2 === `
` ? c2.push("\\n") : d2 === "\r" ? c2.push("\\r") : c2.push(d2);
    return c2.join("");
  }, r2 = t.length > 0 ? o.length + 1 : o.length, s2 = new Array(r2), a = 0;
...
var Ce2 = (e) => {
  ...
  let n = new Array(o.length + 2), r2 = 0, s2 = t.map((l) => Ue2(l));
  n[r2] = `| ${s2.join(" | ")} |`, r2 += 1;
  ...
  for (let l of o) if (l.length < t.length) {
    ...
    for (let c2 = 0; c2 < t.length; c2 += 1) i[c2] = c2 < l.length ? Ue2(l[c2]) : "";
    n[r2] = `| ${i.join(" | ")} |`, r2 += 1;
  } else {
    let i = l.map((c2) => Ue2(c2));
    n[r2] = `| ${i.join(" | ")} |`, r2 += 1;
  }
  return n.join(`
`);
};
var Le2 = ({ children: e, className: t, onCopy: o, onError: n, timeout: r2 = 2e3 }) => {
  let s2 = C3(), [a, l] = useState(false), [i, c2] = useState(false), d2 = useRef(null), p2 = useRef(0), { isAnimating: m3, controls: u4 } = useContext(S2), f2 = B3(), b3 = Re2(u4), w3 = async (g3) => {
    var x3, I3;
    if (typeof window == "undefined" || !((x3 = navigator == null ? void 0 : navigator.clipboard) != null && x3.write)) {
      n == null || n(new Error("Clipboard API not available"));
      return;
    }
    try {
      let N3 = (I3 = d2.current) == null ? void 0 : I3.closest('[data-streamdown="table-wrapper"]'), T3 = N3 == null ? void 0 : N3.querySelector("table");
      if (!T3) {
        n == null || n(new Error("Table not found"));
        return;
      }
      let M3 = we2(T3), E3 = "";
      g3 === "csv" ? E3 = ce2(M3, b3) : g3 === "tsv" ? E3 = Lt(M3) : E3 = Ce2(M3);
      let q3 = new ClipboardItem({ "text/plain": new Blob([E3], { type: "text/plain" }), "text/html": new Blob([T3.outerHTML], { type: "text/html" }) });
      await navigator.clipboard.write([q3]), c2(true), l(false), o == null || o(g3), p2.current = window.setTimeout(() => c2(false), r2);
    } catch (N3) {
      n == null || n(N3);
    }
  };
  useEffect(() => {
    let g3 = (x3) => {
      let I3 = x3.composedPath();
      d2.current && !I3.includes(d2.current) && l(false);
    };
    return document.addEventListener("mousedown", g3), () => {
      document.removeEventListener("mousedown", g3), window.clearTimeout(p2.current);
    };
  }, []);
  let y3 = H3(), h3 = i ? y3.CheckIcon : y3.CopyIcon;
  return jsxs("div", { className: s2("relative"), ref: d2, children: [jsx("button", { className: s2("cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50", t), disabled: m3, onClick: () => l(!a), title: f2.copyTable, type: "button", children: e != null ? e : jsx(h3, { height: 14, width: 14 }) }), a ? jsxs("div", { className: s2("absolute top-full right-0 z-20 mt-1 min-w-[120px] overflow-hidden rounded-md border border-border bg-background shadow-lg"), children: [jsx("button", { className: s2("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => w3("md"), title: f2.copyTableAsMarkdown, type: "button", children: f2.tableFormatMarkdown }), jsx("button", { className: s2("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => w3("csv"), title: f2.copyTableAsCsv, type: "button", children: f2.tableFormatCsv }), jsx("button", { className: s2("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => w3("tsv"), title: f2.copyTableAsTsv, type: "button", children: f2.tableFormatTsv })] }) : null] });
};
var De2 = ({ children: e, className: t, onDownload: o, onError: n }) => {
  let r2 = C3(), [s2, a] = useState(false), l = useRef(null), { isAnimating: i, controls: c2 } = useContext(S2), d2 = B3(), p2 = H3(), m3 = Re2(c2), u4 = (f2) => {
    var b3;
    try {
      let w3 = (b3 = l.current) == null ? void 0 : b3.closest('[data-streamdown="table-wrapper"]'), y3 = w3 == null ? void 0 : w3.querySelector("table");
      if (!y3) {
        n == null || n(new Error("Table not found"));
        return;
      }
      let h3 = we2(y3), g3 = f2 === "csv" ? ce2(h3, m3) : Ce2(h3), x3 = f2 === "csv" ? "csv" : "md", I3 = `${ie2(c2, "table", "table")}.${x3}`;
      J3(I3, g3, f2 === "csv" ? "text/csv" : "text/markdown"), a(false), o == null || o(f2);
    } catch (w3) {
      n == null || n(w3);
    }
  };
  return useEffect(() => {
    let f2 = (b3) => {
      let w3 = b3.composedPath();
      l.current && !w3.includes(l.current) && a(false);
    };
    return document.addEventListener("mousedown", f2), () => {
      document.removeEventListener("mousedown", f2);
    };
  }, []), jsxs("div", { className: r2("relative"), ref: l, children: [jsx("button", { className: r2("cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50", t), disabled: i, onClick: () => a(!s2), title: d2.downloadTable, type: "button", children: e != null ? e : jsx(p2.DownloadIcon, { size: 14 }) }), s2 ? jsxs("div", { className: r2("absolute top-full right-0 z-20 mt-1 min-w-[120px] overflow-hidden rounded-md border border-border bg-background shadow-lg"), children: [jsx("button", { className: r2("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => u4("csv"), title: d2.downloadTableAsCsv, type: "button", children: d2.tableFormatCsv }), jsx("button", { className: r2("w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted/40"), onClick: () => u4("markdown"), title: d2.downloadTableAsMarkdown, type: "button", children: d2.tableFormatMarkdown })] }) : null] });
};
var Wo = ({ children: e, className: t, showCopy: o = true, showDownload: n = true }) => {
  let { Maximize2Icon: r2, XIcon: s2 } = H3(), a = C3(), [l, i] = useState(false), { isAnimating: c2 } = useContext(S2), d2 = B3(), p2 = () => {
    i(true);
  }, m3 = () => {
    i(false);
  };
  return useEffect(() => {
    if (l) {
      ...
      return document.addEventListener("keydown", u4), () => {
        document.removeEventListener("keydown", u4), he2();
      };
    }
  }, [l]), jsxs(Fragment, { children: [jsx("button", { className: a("cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50", t), disabled: c2, onClick: p2, title: d2.viewFullscreen, type: "button", children: jsx(r2, { size: 14 }) }), l ? createPortal(jsx("div", { "aria-label": d2.viewFullscreen, "aria-modal": "true", className: a("fixed inset-0 z-50 flex flex-col bg-background"), "data-streamdown": "table-fullscreen", onClick: m3, onKeyDown: (u4) => {
    u4.key === "Escape" && m3();
  }, role: "dialog", children: jsxs("div", { className: a("flex h-full flex-col"), "data-streamdown": "table-wrapper", onClick: (u4) => u4.stopPropagation(), onKeyDown: (u4) => u4.stopPropagation(), role: "presentation", children: [jsxs("div", { className: a("flex items-center justify-end gap-1 p-4"), children: [o ? jsx(Le2, {}) : null, n ? jsx(De2, {}) : null, jsx("button", { className: a("rounded-md p-1 text-muted-foreground transition-all hover:bg-muted hover:text-foreground"), onClick: m3, title: d2.exitFullscreen, type: "button", children: jsx(s2, { size: 20 }) })] }), jsx("div", { className: a("flex-1 overflow-auto p-4 pt-0 [&_thead]:sticky [&_thead]:top-0 [&_thead]:z-10"), children: jsx("table", { className: a("w-full border-collapse border border-border"), "data-streamdown": "table", children: e }) })] }) }), document.body) : null] });
};
var Xo = ({ children: e, className: t, maxHeight: o, showControls: n, showCopy: r2 = true, showDownload: s2 = true, showFullscreen: a = true, ...l }) => {
  let i = C3(), { isAnimating: c2 } = useContext(S2), d2 = ze2(o), p2 = $e2(c2, !!d2, e), m3 = n && r2, u4 = n && s2, f2 = n && a, b3 = m3 || u4 || f2;
  return jsxs("div", { className: i("my-4 flex flex-col gap-2 rounded-lg border border-border bg-sidebar p-2"), "data-streamdown": "table-wrapper", children: [b3 ? jsxs("div", { className: i("flex items-center justify-end gap-1"), children: [m3 ? jsx(Le2, {}) : null, u4 ? jsx(De2, {}) : null, f2 ? jsx(Wo, { showCopy: m3, showDownload: u4, children: e }) : null] }) : null, jsx("div", { className: i("border-collapse overflow-x-auto overflow-y-auto rounded-md border border-border bg-background"), ref: p2, style: d2 ? { maxHeight: d2 } : void 0, children: jsx("table", { className: i("w-full divide-y divide-border", t), "data-streamdown": "table", ...l, children: e }) })] });
};
var Fs = /startLine=(\d+)/;
var js = /\bnoLineNumbers\b/;
var _s = lazy(() => import("./mermaid-HWGCJPDP-3QVJBIDR.js").then((e) => ({ default: e.Mermaid })));
var zs = /language-([^\s]+)/;
var $s = "node";
function D3(e, t) {
  let o = Object.keys(e);
  ...
  let n = e, r2 = t;
  for (let s2 of o) if (s2 !== $s && !Object.is(n[s2], r2[s2])) return false;
  return true;
}
function qs(e, t) {
  var o, n;
...
var Qe2 = (e, t) => typeof e == "boolean" ? e : e[t] !== false;
var Ht = (e, t) => {
...
var Bt = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("ol", { className: r2("list-inside list-decimal whitespace-normal [li_&]:pl-6", t), "data-streamdown": "ordered-list", ...n, children: e });
}, (e, t) => D3(e, t));
Bt.displayName = "MarkdownOl";
var Uo = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("li", { className: r2("py-1 [&>p]:inline", t), "data-streamdown": "list-item", ...n, children: e });
}, (e, t) => D3(e, t));
Uo.displayName = "MarkdownLi";
var Go = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("ul", { className: r2("list-inside list-disc whitespace-normal [li_&]:pl-6", t), "data-streamdown": "unordered-list", ...n, children: e });
}, (e, t) => D3(e, t));
Go.displayName = "MarkdownUl";
var Yo = memo(({ className: e, node: t, ...o }) => {
  let n = C3();
  return jsx("hr", { className: n("my-6 border-border", e), "data-streamdown": "horizontal-rule", ...o });
}, (e, t) => D3(e, t));
Yo.displayName = "MarkdownHr";
var Qo = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("span", { className: r2("font-semibold", t), "data-streamdown": "strong", ...n, children: e });
}, (e, t) => D3(e, t));
Qo.displayName = "MarkdownStrong";
var Zs = ({ children: e, className: t, href: o, node: n, ...r2 }) => {
  let s2 = C3(), { linkSafety: a } = useContext(S2), [l, i] = useState(false), c2 = o === "streamdown:incomplete-link", d2 = useCallback(async (f2) => {
    if (!(!(a != null && a.enabled && o) || c2)) {
  ...
  }, [a, o, c2]), p2 = useCallback(() => {
    o && window.open(o, "_blank", "noreferrer");
  }, [o]), m3 = useCallback(() => {
    i(false);
  }, []), u4 = { url: o != null ? o : "", isOpen: l, onClose: m3, onConfirm: p2 };
  return a != null && a.enabled && o ? jsxs(Fragment, { children: [jsx("button", { className: s2("wrap-anywhere appearance-none text-left font-medium text-primary underline", t), "data-incomplete": c2, "data-streamdown": "link", onClick: d2, type: "button", children: e }), a.renderModal ? a.renderModal(u4) : jsx(Lo, { ...u4 })] }) : jsx("a", { className: s2("wrap-anywhere font-medium text-primary underline", t), "data-incomplete": c2, "data-streamdown": "link", href: o, rel: "noreferrer", target: "_blank", ...r2, children: e });
};
var en2 = memo(Zs, (e, t) => D3(e, t));
en2.displayName = "MarkdownA";
var tn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("h1", { className: r2("mt-6 mb-2 font-semibold text-3xl", t), "data-streamdown": "heading-1", ...n, children: e });
}, (e, t) => D3(e, t));
tn2.displayName = "MarkdownH1";
var on2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("h2", { className: r2("mt-6 mb-2 font-semibold text-2xl", t), "data-streamdown": "heading-2", ...n, children: e });
}, (e, t) => D3(e, t));
on2.displayName = "MarkdownH2";
var nn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("h3", { className: r2("mt-6 mb-2 font-semibold text-xl", t), "data-streamdown": "heading-3", ...n, children: e });
}, (e, t) => D3(e, t));
nn2.displayName = "MarkdownH3";
var rn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("h4", { className: r2("mt-6 mb-2 font-semibold text-lg", t), "data-streamdown": "heading-4", ...n, children: e });
}, (e, t) => D3(e, t));
rn2.displayName = "MarkdownH4";
var sn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("h5", { className: r2("mt-6 mb-2 font-semibold text-base", t), "data-streamdown": "heading-5", ...n, children: e });
}, (e, t) => D3(e, t));
sn2.displayName = "MarkdownH5";
var an2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("h6", { className: r2("mt-6 mb-2 font-semibold text-sm", t), "data-streamdown": "heading-6", ...n, children: e });
}, (e, t) => D3(e, t));
an2.displayName = "MarkdownH6";
var ln2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let { controls: r2, tableMaxHeight: s2 } = useContext(S2), a = Qe2(r2, "table"), l = Ht(r2, "copy"), i = Ht(r2, "download"), c2 = Ht(r2, "fullscreen");
  return jsx(Xo, { className: t, maxHeight: s2, showControls: a, showCopy: l, showDownload: i, showFullscreen: c2, ...n, children: e });
}, (e, t) => D3(e, t));
ln2.displayName = "MarkdownTable";
var cn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("thead", { className: r2("bg-muted/80", t), "data-streamdown": "table-header", ...n, children: e });
}, (e, t) => D3(e, t));
cn2.displayName = "MarkdownThead";
var dn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("tbody", { className: r2("divide-y divide-border", t), "data-streamdown": "table-body", ...n, children: e });
}, (e, t) => D3(e, t));
dn2.displayName = "MarkdownTbody";
var mn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("tr", { className: r2("border-border", t), "data-streamdown": "table-row", ...n, children: e });
}, (e, t) => D3(e, t));
mn2.displayName = "MarkdownTr";
var un2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("th", { className: r2("whitespace-nowrap px-4 py-2 text-left font-semibold text-sm", t), "data-streamdown": "table-header-cell", ...n, children: e });
}, (e, t) => D3(e, t));
un2.displayName = "MarkdownTh";
var pn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("td", { className: r2("px-4 py-2 text-sm", t), "data-streamdown": "table-cell", ...n, children: e });
}, (e, t) => D3(e, t));
pn2.displayName = "MarkdownTd";
var fn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("blockquote", { className: r2("my-4 border-muted-foreground/30 border-l-4 pl-4 text-muted-foreground italic", t), "data-streamdown": "blockquote", ...n, children: e });
}, (e, t) => D3(e, t));
fn2.displayName = "MarkdownBlockquote";
var gn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("sup", { className: r2("text-sm", t), "data-streamdown": "superscript", ...n, children: e });
}, (e, t) => D3(e, t));
gn2.displayName = "MarkdownSup";
var bn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  let r2 = C3();
  return jsx("sub", { className: r2("text-sm", t), "data-streamdown": "subscript", ...n, children: e });
}, (e, t) => D3(e, t));
bn2.displayName = "MarkdownSub";
var hn2 = memo(({ children: e, className: t, node: o, ...n }) => {
  if ("data-footnotes" in n) {
    let s2 = (i) => {
      var m3, u4;
      if (!isValidElement(i)) return false;
      let c2 = Array.isArray(i.props.children) ? i.props.children : [i.props.children], d2 = false, p2 = false;
      for (let f2 of c2) if (f2) {
        if (typeof f2 == "string") f2.trim() !== "" && (d2 = true);
        else if (isValidElement(f2)) if (((m3 = f2.props) == null ? void 0 : m3["data-footnote-backref"]) !== void 0) p2 = true;
        else {
          let b3 = Array.isArray(f2.props.children) ? f2.props.children : [f2.props.children];
          for (let w3 of b3) {
            if (typeof w3 == "string" && w3.trim() !== "") {
              d2 = true;
              break;
            }
            if (isValidElement(w3) && ((u4 = w3.props) == null ? void 0 : u4["data-footnote-backref"]) === void 0) {
              d2 = true;
              break;
            }
          }
      ...
      if (i.type === Bt) {
        let d2 = (Array.isArray(i.props.children) ? i.props.children : [i.props.children]).filter((p2) => !s2(p2));
        return d2.length === 0 ? null : { ...i, props: { ...i.props, children: d2 } };
      }
      return i;
    }) : e;
    return (Array.isArray(a) ? a.some((i) => i !== null) : a !== null) ? jsx("section", { className: t, ...n, children: a }) : null;
  }
  return jsx("section", { className: t, ...n, children: e });
}, (e, t) => D3(e, t));
hn2.displayName = "MarkdownSection";
var Xs = ({ node: e, className: t, children: o, ...n }) => {
  var q3, _2;
  let r2 = C3(), s2 = !("data-block" in n), { mermaid: a, controls: l, lineNumbers: i } = useContext(S2), c2 = ye2(), d2 = ht(), p2 = t == null ? void 0 : t.match(zs), m3 = (q3 = p2 == null ? void 0 : p2.at(1)) != null ? q3 : "", u4 = Do(m3);
  if (s2) return jsx("code", { className: r2("rounded bg-muted px-1.5 py-0.5 font-mono text-sm", t), "data-streamdown": "inline-code", ...n, children: o });
  let f2 = (_2 = e == null ? void 0 : e.properties) == null ? void 0 : _2.metastring, b3 = f2 == null ? void 0 : f2.match(Fs), w3 = b3 ? Number.parseInt(b3[1], 10) : void 0, y3 = w3 !== void 0 && w3 >= 1 ? w3 : void 0, g3 = !(f2 ? js.test(f2) : false) && i !== false, x3 = "";
  if (isValidElement(o) && o.props && typeof o.props == "object" && "children" in o.props && typeof o.props.children == "string" ? x3 = o.props.children : typeof o == "string" && (x3 = o), u4) {
    let W3 = u4.component;
    return jsx(Suspense, { fallback: jsx(Xe2, {}), children: jsx(W3, { code: x3, isIncomplete: d2, language: m3, meta: f2 }) });
  }
  if (m3 === "mermaid" && c2) {
    let W3 = Qe2(l, "mermaid"), X3 = Ge2(l, "download"), K3 = Ge2(l, "copy"), j3 = Ge2(l, "fullscreen"), O2 = Ge2(l, "panZoom"), P3 = W3 && (X3 || K3 || j3);
    return jsx(Suspense, { fallback: jsx(Xe2, {}), children: jsxs("div", { className: r2("group relative my-4 flex w-full flex-col gap-2 rounded-xl border border-border bg-sidebar p-2", t), "data-streamdown": "mermaid-block", children: [jsx("div", { className: r2("flex h-8 items-center text-muted-foreground text-xs"), children: jsx("span", { className: r2("ml-1 font-mono lowercase"), children: "mermaid" }) }), P3 ? jsx("div", { className: r2("pointer-events-none sticky top-2 z-10 -mt-10 flex h-8 items-center justify-end"), children: jsxs("div", { className: r2("pointer-events-auto flex shrink-0 items-center gap-2 rounded-md border border-sidebar bg-sidebar/80 px-1.5 py-1 supports-[backdrop-filter]:bg-sidebar/70 supports-[backdrop-filter]:backdrop-blur"), "data-streamdown": "mermaid-block-actions", children: [X3 ? jsx(Je2, { chart: x3, config: a == null ? void 0 : a.config }) : null, K3 ? jsx(ge2, { code: x3 }) : null, j3 ? jsx(Fo, { chart: x3, config: a == null ? void 0 : a.config }) : null] }) }) : null, jsx("div", { className: r2("rounded-md border border-border bg-background"), children: jsx(_s, { chart: x3, config: a == null ? void 0 : a.config, showControls: O2 }) })] }) });
  }
  let I3 = Qe2(l, "code"), N3 = Jo(l, "download"), T3 = Jo(l, "copy"), { "data-block": M3, ...E3 } = n;
  return jsx(Tt, { className: t, code: x3, isIncomplete: d2, language: m3, lineNumbers: g3, startLine: y3, ...E3, children: I3 ? jsxs(Fragment, { children: [N3 ? jsx(It, { code: x3, language: m3 }) : null, T3 ? jsx(ge2, {}) : null] }) : null });
};
var yn2 = memo(Xs, (e, t) => D3(e, t) && qs(e.node, t.node));
yn2.displayName = "MarkdownCode";
var Ks = ({ node: e, className: t, ...o }) => {
  let { controls: n } = useContext(S2), r2 = Qe2(n, "image"), s2 = r2 && Ws(n, "download");
  return jsx(So, { className: t, node: e, showControls: r2, showDownloadControl: s2, ...o });
};
var wn2 = memo(Ks, (e, t) => D3(e, t));
wn2.displayName = "MarkdownImg";
var Cn = memo(({ children: e, node: t, ...o }) => {
  let r2 = (Array.isArray(e) ? e : [e]).filter((s2) => s2 != null && s2 !== "");
  if (r2.length === 1 && isValidElement(r2[0])) {
    let s2 = r2[0].props.node, a = s2 == null ? void 0 : s2.tagName;
    if (a === "img") return jsx(Fragment, { children: e });
    if (a === "code" && "data-block" in r2[0].props) return jsx(Fragment, { children: e });
  }
  return jsx("p", { ...o, children: e });
}, (e, t) => D3(e, t));
Cn.displayName = "MarkdownParagraph";
var xn2 = { ol: Bt, li: Uo, ul: Go, hr: Yo, strong: Qo, a: en2, h1: tn2, h2: on2, h3: nn2, h4: rn2, h5: sn2, h6: an2, table: ln2, thead: cn2, tbody: dn2, tr: mn2, th: un2, td: pn2, blockquote: fn2, code: yn2, img: wn2, pre: ({ children: e }) => isValidElement(e) ? cloneElement(e, { "data-block": "true" }) : e, sup: gn2, sub: bn2, p: Cn, section: hn2 };
var Js = /[\u0590-\u08FF\uFB1D-\uFDFF\uFE70-\uFEFF]/;
var Us = /\p{L}/u;
function He2(e) {
  let t = e.replace(/(```|~~~)[\s\S]*?\1/g, "").replace(/^#{1,6}\s+/gm, "").replace(/(\*{1,3}|_{1,3})/g, "").replace(/`[^`]*`/g, "").replace(/\[([^\]]*)\]\([^)]*\)/g, "$1").replace(/^[\s>*\-+\d.]+/gm, ""), o, n = 0, r2 = 0;
  for (let s2 of t) {
    if (Js.test(s2)) {
      o != null || (o = "rtl"), r2 += 1;
      continue;
    }
    Us.test(s2) && (o != null || (o = "ltr"), n += 1);
  }
  return r2 > n ? "rtl" : n > r2 ? "ltr" : o != null ? o : "ltr";
}
var Gs = /^[ \t]{0,3}(`{3,}|~{3,})/;
var Ys = /^\|?[ \t]*:?-{1,}:?[ \t]*(\|[ \t]*:?-{1,}:?[ \t]*)*\|?$/;
var Ot = (e) => {
...
var Pn2 = [];
var Mn2 = { allowDangerousHtml: true };
...
var Ft2 = class {
  ...
  generateCacheKey(t) {
    ...
    if (o) return o;
    let n = t.rehypePlugins, r2 = t.remarkPlugins, s2 = t.remarkRehypeOptions;
    if (!(n || r2 || s2)) {
      let p2 = "default";
      ...
      for (let u4 = 0; u4 < p2.length; u4 += 1) {
        ...
        if (u4 > 0 && (m3 += ","), Array.isArray(f2)) {
          ...
          if (typeof b3 == "function") {
            let y3 = ot.get(b3);
            y3 || (y3 = b3.name, ot.set(b3, y3)), m3 += y3;
          } else m3 += String(b3);
          m3 += ":", m3 += JSON.stringify(w3);
        } else if (typeof f2 == "function") {
          let b3 = ot.get(f2);
          b3 || (b3 = f2.name, ot.set(f2, b3)), m3 += b3;
        } else m3 += String(f2);
      }
      return m3;
    }, l = a(n), i = a(r2), c2 = s2 ? JSON.stringify(s2) : "", d2 = `${i}::${l}::${c2}`;
    return this.keyCache.set(t, d2), d2;
  }
  get(t) {
...
var In2 = new Ft2();
var jt2 = (e) => {
  let t = la(e), o = e.children || "", n = t.runSync(t.parse(o), o);
  return fa(n, e);
};
var la = (e) => {
  let t = In2.get(e);
  if (t) return t;
  let o = da(e);
  return In2.set(e, o), o;
};
var ca = (e) => e.some((t) => Array.isArray(t) ? t[0] === rehypeRaw : t === rehypeRaw);
var da = (e) => {
  let t = e.rehypePlugins || Pn2, o = e.remarkPlugins || Pn2, n = ca(t) ? o : [...o, vn2], r2 = e.remarkRehypeOptions ? { ...Mn2, ...e.remarkRehypeOptions } : Mn2;
  return unified().use(remarkParse).use(n).use(remarkRehype, r2).use(t);
};
var Nn2 = (e) => e;
var ma = (e, t, o, n) => {
...
var ua = (e, t) => {
  ...
  for (let n in urlAttributes) if (Object.hasOwn(urlAttributes, n) && Object.hasOwn(e.properties, n)) {
    let r2 = e.properties[n], s2 = urlAttributes[n];
    (s2 === null || s2.includes(e.tagName)) && (e.properties[n] = (o = t(String(r2 || ""), n, e)) != null ? o : void 0);
  }
};
var pa = (e, t, o, n, r2, s2) => {
  let a = false;
  return n ? a = !n.includes(e.tagName) : r2 && (a = r2.includes(e.tagName)), !a && s2 && typeof t == "number" && (a = !s2(e, t, o)), a;
};
var fa = (e, t) => {
  let { allowElement: o, allowedElements: n, disallowedElements: r2, skipHtml: s2, unwrapDisallowed: a, urlTransform: l } = t;
  if (o || n || r2 || s2 || l) {
    let c2 = l || Nn2;
    visit(e, (d2, p2, m3) => {
      if (d2.type === "raw" && m3 && typeof p2 == "number") return ma(m3, p2, s2, d2.value), p2;
      if (d2.type === "element" && (ua(d2, c2), pa(d2, p2, m3, n, r2, o) && m3 && typeof p2 == "number")) return a && d2.children ? m3.children.splice(p2, 1, ...d2.children) : m3.children.splice(p2, 1), p2;
    });
  }
  return toJsxRuntime(e, { Fragment, components: t.components, ignoreInvalidStyle: true, jsx, jsxs, passKeys: true, passNode: true });
};
var ba = /\[\^[\w-]{1,200}\](?!:)/;
var ha = /\[\^[\w-]{1,200}\]:/;
var ya = /<([A-Za-z][\w:-]*)[\s>/]/;
var wa = /* @__PURE__ */ new Set(["area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta", "param", "source", "track", "wbr"]);
var Sn2 = /* @__PURE__ */ new Map();
var Rn2 = /* @__PURE__ */ new Map();
var Ca = (e) => {
  let t = e.toLowerCase(), o = Sn2.get(t);
  if (o) return o;
  let n = new RegExp(`<${t}(?=[\\s>/])[^>]*>`, "gi");
  return Sn2.set(t, n), n;
};
var xa = (e) => {
...
var En2 = (e, t) => {
  ...
  if (!o) return 0;
  let n = 0;
  for (let r2 of o) r2.trimEnd().endsWith("/>") || (n += 1);
  return n;
};
var Ln2 = (e, t) => {
  let o = e.match(xa(t));
...
var _t = (e) => {
  ...
  if (t || o) return [e];
  let n = x2.lex(e, { gfm: true }), r2 = [], s2 = [], a = false;
  for (let l of n) {
    let i = l.raw, c2 = r2.length;
    if (s2.length > 0) {
      r2[c2 - 1] += i;
      let d2 = s2.at(-1), p2 = En2(i, d2), m3 = Ln2(i, d2);
      for (let u4 = 0; u4 < p2; u4 += 1) s2.push(d2);
      for (let u4 = 0; u4 < m3; u4 += 1) s2.length > 0 && s2.at(-1) === d2 && s2.pop();
      continue;
    }
    if (l.type === "html" && l.block) {
      let d2 = i.match(ya);
      if (d2) {
        let p2 = d2[1], m3 = En2(i, p2), u4 = Ln2(i, p2);
        m3 > u4 && s2.push(p2);
      }
    }
    if (c2 > 0 && !a) {
    ...
    r2.push(i), l.type !== "space" && (a = l.type === "code");
  }
  return r2;
};
var va = /^\n*/;
...
var Ma = (e, t) => {
  let o = new RegExp(`<(${t})(?=[\\s>/])([^>]*)>`, "gi"), n = "", r2 = 0, s2 = o.exec(e);
  for (; s2; ) {
    let a = s2[0], l = s2[1], i = s2.index + a.length, c2 = e.slice(i);
    if (new RegExp(`</${l}\\s*>`, "i").test(c2)) {
      s2 = o.exec(e);
      continue;
    }
    if (n += e.slice(r2, s2.index), c2.length === 0) {
      n += a, r2 = i, s2 = o.exec(e);
      continue;
    }
    if (c2.startsWith(`

`)) {
      let u4 = c2.slice(2).replace(zt, `
<!---->
`);
      n += `${a}
...
${m3}`, r2 = e.length;
  ...
  return r2 === 0 ? e : n + e.slice(r2);
};
var Dn2 = (e, t) => {
  if (!t.length) return e;
  let o = e;
  for (let n of t) {
    let r2 = new RegExp(`(<${n}(?=[\\s>/])[^>]*>)([\\s\\S]*?)(</${n}\\s*>)`, "gi");
    o = o.replace(r2, (s2, a, l, i) => Pa(a, l, i)), o = Ma(o, n);
  }
  return o;
};
var Ia = /([\\`*_~[\]|])/g;
...
var Ra = /* @__PURE__ */ new Set(["blockquote", "dd", "dt", "figcaption", "h1", "h2", "h3", "h4", "h5", "h6", "li", "p", "td", "th"]);
var Hn2 = /* @__PURE__ */ new Set(["code", "kbd", "pre", "samp", "var"]);
function Bn2(e) {
  return e.children.map((t) => t.type === "text" ? t.value : t.type === "element" && !Hn2.has(t.tagName) ? Bn2(t) : "").join("");
}
function On2() {
  return (e) => {
    visit(e, "element", (t) => {
      if (Hn2.has(t.tagName)) {
        t.properties != null || (t.properties = {}), t.properties.dir = "ltr";
        return;
      }
      Ra.has(t.tagName) && (t.properties != null || (t.properties = {}), typeof t.properties.dir != "string" && (t.properties.dir = He2(Bn2(t))));
    });
  };
}
var Vn2 = (e) => e.type === "text" ? e.value : "children" in e && Array.isArray(e.children) ? e.children.map(Vn2).join("") : "";
var Fn2 = (e) => (t) => {
  ...
  visit(t, "element", (n) => {
    if (o.has(n.tagName.toLowerCase())) {
      let r2 = Vn2(n);
      n.children = r2 ? [{ type: "text", value: r2 }] : [];
    }
  });
};
var jn2 = () => (e) => {
...
var ja = /^[ \t]*<[\w!/?-]/;
var _a2 = /(^|\n)[ \t]{4,}(?=<[\w!/?-])/g;
var za = (e) => typeof e != "string" || e.length === 0 || !ja.test(e) ? e : e.replace(_a2, "$1");
var $n2;
var qn2;
var Wn2;
var Zn2;
var nt2 = { ...defaultSchema, clobberPrefix: "", protocols: { ...defaultSchema.protocols, href: [...(qn2 = ($n2 = defaultSchema.protocols) == null ? void 0 : $n2.href) != null ? qn2 : [], "tel", "streamdown"] }, attributes: { ...defaultSchema.attributes, code: [...(Zn2 = (Wn2 = defaultSchema.attributes) == null ? void 0 : Wn2.code) != null ? Zn2 : [], "metastring"] } };
var $t = { raw: rehypeRaw, sanitize: [rehypeSanitize, nt2], harden: [harden, { allowedImagePrefixes: ["*"], allowedLinkPrefixes: ["*"], allowedProtocols: ["*"], defaultOrigin: void 0, allowDataImages: true }] };
var $a = { gfm: [remarkGfm, {}], codeMeta: jn2 };
var zn2 = Object.values($t);
var qa = Object.values($a);
var Wa = { block: " \u258B", circle: " \u25CF" };
var Un2 = ["github-light", "github-dark"];
var Gn2 = { enabled: true };
var Za = { codeBlockMaxHeight: 400, shikiTheme: Un2, controls: true, isAnimating: false, lineNumbers: true, mode: "streaming", mermaid: void 0, linkSafety: Gn2, tableMaxHeight: 300 };
var S2 = createContext(Za);
var Yn2 = memo(({ content: e, shouldParseIncompleteMarkdown: t, shouldNormalizeHtmlIndentation: o, index: n, isIncomplete: r2, dir: s2, animatePlugin: a, ...l }) => {
  useLayoutEffect(() => {
    a == null || a.commit();
  });
  let i = typeof e == "string" && o ? za(e) : e, c2 = jsx(jt2, { ...l, children: i });
  return jsx(bt.Provider, { value: r2, children: s2 ? jsx("div", { dir: s2, style: { display: "contents" }, children: c2 }) : c2 });
}, (e, t) => {
  ...
  if (e.components !== t.components) {
    ...
    if (o.length !== n.length || o.some((r2) => {
      var s2, a;
      return ((s2 = e.components) == null ? void 0 : s2[r2]) !== ((a = t.components) == null ? void 0 : a[r2]);
    })) return false;
  }
  return !(e.rehypePlugins !== t.rehypePlugins || e.remarkPlugins !== t.remarkPlugins || !!e.animatePlugin != !!t.animatePlugin);
});
Yn2.displayName = "Block";
var Xa = memo(({ children: e, mode: t = "streaming", dir: o, parseIncompleteMarkdown: n = true, normalizeHtmlIndentation: r2 = false, components: s2, rehypePlugins: a = zn2, remarkPlugins: l = qa, className: i, shikiTheme: c2, mermaid: d2, codeBlockMaxHeight: p2 = 400, controls: m3 = true, isAnimating: u4 = false, tableMaxHeight: f2 = 300, animated: b3, BlockComponent: w3 = Yn2, parseMarkdownIntoBlocksFn: y3 = _t, caret: h3, plugins: g3, remend: x3, linkSafety: I3 = Gn2, lineNumbers: N3 = true, allowedTags: T3, literalTagContent: M3, translations: E3, icons: q3, prefix: _2, onAnimationStart: W3, onAnimationEnd: X3, ...K3 }) => {
  let j3 = useId(), O2 = useMemo(() => lo(_2), [_2]), P3 = useRef(null), V3 = useRef(W3), re3 = useRef(X3);
  V3.current = W3, re3.current = X3, useEffect(() => {
    var A2, Z2, Q3;
    if (t === "static") return;
    let k3 = P3.current;
    if (P3.current = u4, k3 === null) {
      u4 && ((A2 = V3.current) == null || A2.call(V3));
      return;
    }
    u4 && !k3 ? (Z2 = V3.current) == null || Z2.call(V3) : !u4 && k3 && ((Q3 = re3.current) == null || Q3.call(re3));
  }, [u4, t]);
  let st = useMemo(() => T3 ? Object.keys(T3) : [], [T3]), at = useMemo(() => {
    if (typeof e != "string") return "";
    let k3 = t === "streaming" && n ? We(e, x3) : e;
    return M3 && M3.length > 0 && (k3 = An2(k3, M3)), st.length > 0 && (k3 = Dn2(k3, st)), k3;
  }, [e, t, n, x3, st, M3]), G3 = useMemo(() => y3(at), [at, y3]), it = useMemo(() => o === "auto" ? G3.map(He2) : void 0, [G3, o]), nr = useMemo(() => G3.map((k3, A2) => `${j3}-${A2}`), [G3.length, j3]), ue2 = useMemo(() => b3 === true ? "true" : b3 ? JSON.stringify(b3) : "", [b3]), Y2 = useRef(null), ke2 = useRef([]), se2 = useRef([]), qt = useRef(null), lt = useRef("");
  if (ue2) {
    if (lt.current !== ue2) {
      lt.current = ue2;
      let k3 = ue2 !== "true" ? b3.maxBacklogMs : void 0;
      Y2.current = pt({ maxBacklogMs: k3 }), ke2.current = [], se2.current = [];
    } else Y2.current || (Y2.current = pt());
    u4 && Y2.current && Y2.current.beginPass(Y2.current.now());
  } else Y2.current = null, ke2.current = [], se2.current = [], lt.current = "";
  useLayoutEffect(() => {
    var k3;
    u4 && ((k3 = Y2.current) == null || k3.commitPass());
  });
  let Wt = useMemo(() => {
    var k3, A2;
    return { codeBlockMaxHeight: p2, shikiTheme: (A2 = c2 != null ? c2 : (k3 = g3 == null ? void 0 : g3.code) == null ? void 0 : k3.getThemes()) != null ? A2 : Un2, controls: m3, isAnimating: u4, lineNumbers: N3, mode: t, mermaid: d2, linkSafety: I3, tableMaxHeight: f2 };
  }, [p2, c2, m3, u4, N3, t, d2, I3, g3 == null ? void 0 : g3.code, f2]), rr = useMemo(() => E3 ? JSON.stringify(E3) : "", [E3]), Zt = useMemo(() => ({ ...We3, ...E3 }), [rr]), Xt2 = useMemo(() => {
    let { inlineCode: k3, ...A2 } = s2 != null ? s2 : {}, Z2 = { ...xn2, ...A2 };
    if (k3) {
      let Q3 = Z2.code;
      Z2.code = (pe2) => "data-block" in pe2 ? Q3 ? createElement(Q3, pe2) : null : createElement(k3, pe2);
    }
    return Z2;
  }, [s2]), Kt2 = useMemo(() => {
    let k3 = [];
  ...
  }, [l, g3 == null ? void 0 : g3.math, g3 == null ? void 0 : g3.cjk]), ve2 = useMemo(() => {
    var A2;
    let k3 = a;
    if (T3 && Object.keys(T3).length > 0 && a === zn2) {
      let Z2 = { ...nt2, tagNames: [...(A2 = nt2.tagNames) != null ? A2 : [], ...Object.keys(T3)], attributes: { ...nt2.attributes, ...T3 } };
      k3 = [$t.raw, [rehypeSanitize, Z2], $t.harden];
    }
    return M3 && M3.length > 0 && (k3 = [...k3, [Fn2, M3]]), g3 != null && g3.math && (k3 = [...k3, g3.math.rehypePlugin]), o === "auto" && t === "static" && (k3 = [...k3, On2]), k3;
  }, [a, g3 == null ? void 0 : g3.math, T3, M3, o, t]), ct = useMemo(() => {
    if (!u4 || G3.length === 0) return false;
    let k3 = G3.at(-1);
    return Ot(k3) || kn2(k3);
  }, [u4, G3]), sr = useMemo(() => h3 && u4 && !ct ? { "--streamdown-caret": `"${Wa[h3]}"` } : void 0, [h3, u4, ct]), ar = (k3) => {
    let A2 = null;
    if (Y2.current && u4) {
      if (!ke2.current[k3]) {
        let Q3 = ue2 && ue2 !== "true" ? b3 : {}, { maxBacklogMs: pe2, ...Fe2 } = Q3;
        ke2.current[k3] = je2({ ...Fe2, timeline: Y2.current });
      }
      A2 = ke2.current[k3];
    }
    qt.current !== ve2 && (se2.current = [], qt.current = ve2), A2 && !se2.current[k3] && (se2.current[k3] = [...ve2, A2.rehypePlugin]);
    let Z2 = A2 && se2.current[k3] ? se2.current[k3] : ve2;
    return { blockAnimatePlugin: A2, blockRehypePlugins: Z2 };
  };
  return t === "static" ? jsx(Ze2.Provider, { value: Zt, children: jsx(Ke2.Provider, { value: g3 != null ? g3 : null, children: jsx(S2.Provider, { value: Wt, children: jsx(Pt, { icons: q3, children: jsx(_e2.Provider, { value: O2, children: jsx("div", { className: O2("space-y-4 whitespace-normal [&>*:first-child]:mt-0 [&>*:last-child]:mb-0", i), dir: o === "auto" ? void 0 : o, children: jsx(jt2, { components: Xt2, rehypePlugins: ve2, remarkPlugins: Kt2, ...K3, children: at }) }) }) }) }) }) }) : jsx(Ze2.Provider, { value: Zt, children: jsx(Ke2.Provider, { value: g3 != null ? g3 : null, children: jsx(S2.Provider, { value: Wt, children: jsx(Pt, { icons: q3, children: jsx(_e2.Provider, { value: O2, children: jsxs("div", { className: O2("space-y-4 whitespace-normal [&>*:first-child]:mt-0 [&>*:last-child]:mb-0", h3 && !ct ? "[&>*:last-child]:after:inline [&>*:last-child]:after:align-baseline [&>*:last-child]:after:content-[var(--streamdown-caret)]" : null, i), style: sr, children: [G3.length === 0 && h3 && u4 && jsx("span", {}), G3.map((k3, A2) => {
    var Jt2;
    let Z2 = A2 === G3.length - 1, Q3 = u4 && Z2 && Ot(k3), { blockAnimatePlugin: pe2, blockRehypePlugins: Fe2 } = ar(A2);
    return jsx(w3, { animatePlugin: pe2, components: Xt2, content: k3, dir: (Jt2 = it == null ? void 0 : it[A2]) != null ? Jt2 : o !== "auto" ? o : void 0, index: A2, isIncomplete: Q3, rehypePlugins: Fe2, remarkPlugins: Kt2, shouldNormalizeHtmlIndentation: r2, shouldParseIncompleteMarkdown: n, ...K3 }, nr[A2]);
  })] }) }) }) }) }) });
...
var or = ({ children: e, className: t, minZoom: o = 0.5, maxZoom: n = 3, zoomStep: r2 = 0.1, showControls: s2 = true, initialZoom: a = 1, fullscreen: l = false }) => {
  let { RotateCcwIcon: i, ZoomInIcon: c2, ZoomOutIcon: d2 } = H3(), p2 = C3(), m3 = B3(), u4 = useRef(null), f2 = useRef(null), [b3, w3] = useState(a), [y3, h3] = useState({ x: 0, y: 0 }), [g3, x3] = useState(false), [I3, N3] = useState({ x: 0, y: 0 }), [T3, M3] = useState({ x: 0, y: 0 }), E3 = useCallback((P3) => {
    w3((V3) => Math.max(o, Math.min(n, V3 + P3)));
  }, [o, n]), q3 = useCallback(() => {
    E3(r2);
  }, [E3, r2]), _2 = useCallback(() => {
    E3(-r2);
  }, [E3, r2]), W3 = useCallback(() => {
    w3(a), h3({ x: 0, y: 0 });
  }, [a]), X3 = useCallback((P3) => {
    P3.preventDefault();
    let V3 = P3.deltaY > 0 ? -r2 : r2;
    E3(V3);
  }, [E3, r2]), K3 = useCallback((P3) => {
    if (P3.button !== 0 || P3.isPrimary === false) return;
    x3(true), N3({ x: P3.clientX, y: P3.clientY }), M3(y3);
    let V3 = P3.currentTarget;
    V3 instanceof HTMLElement && V3.setPointerCapture(P3.pointerId);
  }, [y3]), j3 = useCallback((P3) => {
    if (!g3) return;
    P3.preventDefault();
    let V3 = P3.clientX - I3.x, re3 = P3.clientY - I3.y;
    h3({ x: T3.x + V3, y: T3.y + re3 });
  }, [g3, I3, T3]), O2 = useCallback((P3) => {
    x3(false);
    let V3 = P3.currentTarget;
    V3 instanceof HTMLElement && V3.releasePointerCapture(P3.pointerId);
  }, []);
  return useEffect(() => {
    let P3 = u4.current;
    if (P3) return P3.addEventListener("wheel", X3, { passive: false }), () => {
      P3.removeEventListener("wheel", X3);
    };
  }, [X3]), useEffect(() => {
    let P3 = f2.current;
    if (P3 && g3) return document.body.style.userSelect = "none", P3.addEventListener("pointermove", j3, { passive: false }), P3.addEventListener("pointerup", O2), P3.addEventListener("pointercancel", O2), () => {
      document.body.style.userSelect = "", P3.removeEventListener("pointermove", j3), P3.removeEventListener("pointerup", O2), P3.removeEventListener("pointercancel", O2);
    };
  }, [g3, j3, O2]), jsxs("div", { className: p2("relative flex flex-col", l ? "h-full w-full" : "min-h-28 w-full", t), ref: u4, style: { cursor: g3 ? "grabbing" : "grab" }, children: [s2 ? jsxs("div", { className: p2("absolute z-10 flex flex-col gap-1 rounded-md border border-border bg-background/80 p-1 supports-[backdrop-filter]:bg-background/70 supports-[backdrop-filter]:backdrop-blur-sm", l ? "bottom-4 left-4" : "bottom-2 left-2"), children: [jsx("button", { "aria-label": m3.zoomIn, className: p2("flex items-center justify-center rounded p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"), disabled: b3 >= n, onClick: q3, title: m3.zoomIn, type: "button", children: jsx(c2, { "aria-hidden": "true", size: 16 }) }), jsx("button", { "aria-label": m3.zoomOut, className: p2("flex items-center justify-center rounded p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"), disabled: b3 <= o, onClick: _2, title: m3.zoomOut, type: "button", children: jsx(d2, { "aria-hidden": "true", size: 16 }) }), jsx("button", { "aria-label": m3.resetView, className: p2("flex items-center justify-center rounded p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"), onClick: W3, title: m3.resetView, type: "button", children: jsx(i, { "aria-hidden": "true", size: 16 }) })] }) : null, jsx("div", { className: p2("flex-1 origin-center transition-transform duration-150 ease-out", l ? "flex h-full w-full items-center justify-center" : "flex w-full items-center justify-center"), onPointerDown: K3, ref: f2, role: "application", style: { transform: `translate(${y3.x}px, ${y3.y}px) scale(${b3})`, transformOrigin: "center center", touchAction: "none", willChange: "transform" }, children: e })] });
};
var Vo = ({ chart: e, className: t, config: o, fullscreen: n = false, showControls: r2 = true }) => {
  let s2 = C3(), [a, l] = useState(null), [i, c2] = useState(false), [d2, p2] = useState(""), [m3, u4] = useState(""), [f2, b3] = useState(0), { mermaid: w3 } = useContext(S2), y3 = ye2(), h3 = w3 == null ? void 0 : w3.errorComponent, { shouldRender: g3, containerRef: x3 } = Ut2({ immediate: n });
  if (useEffect(() => {
    if (!g3) return;
    if (!y3) {
    ...
    (async () => {
      try {
        l(null), c2(true);
        let T3 = y3.getMermaid(o), M3 = e.split("").reduce((_2, W3) => (_2 << 5) - _2 + W3.charCodeAt(0) | 0, 0), E3 = `mermaid-${Math.abs(M3)}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, { svg: q3 } = await T3.render(E3, e);
        p2(q3), u4(q3);
      } catch (T3) {
        if (!(m3 || d2)) {
          let M3 = T3 instanceof Error ? T3.message : "Failed to render Mermaid chart";
          l(M3);
        }
      } finally {
        c2(false);
  ...
  }, [e, o, f2, g3, y3]), !(g3 || d2 || m3)) return jsx("div", { className: s2("my-4 min-h-[200px]", t), ref: x3 });
  if (i && !d2 && !m3) return jsx("div", { className: s2("my-4 flex justify-center p-4", t), ref: x3, children: jsxs("div", { className: s2("flex items-center space-x-2 text-muted-foreground"), children: [jsx("div", { className: s2("h-4 w-4 animate-spin rounded-full border-current border-b-2") }), jsx("span", { className: s2("text-sm"), children: "Loading diagram..." })] }) });
  if (a && !d2 && !m3) {
    let N3 = () => b3((T3) => T3 + 1);
    return h3 ? jsx("div", { ref: x3, children: jsx(h3, { chart: e, error: a, retry: N3 }) }) : jsxs("div", { className: s2("rounded-md bg-red-50 p-4", t), ref: x3, children: [jsxs("p", { className: s2("font-mono text-red-700 text-sm"), children: ["Mermaid Error: ", a] }), jsxs("details", { className: s2("mt-2"), children: [jsx("summary", { className: s2("cursor-pointer text-red-600 text-xs"), children: "Show Code" }), jsx("pre", { className: s2("mt-2 overflow-x-auto rounded bg-red-100 p-2 text-red-800 text-xs"), children: e })] })] });
  }
  let I3 = d2 || m3;
  return jsx("div", { className: s2("size-full", t), "data-streamdown": "mermaid", ref: x3, children: jsx(or, { className: s2(n ? "size-full overflow-hidden" : "overflow-hidden", t), fullscreen: n, maxZoom: 3, minZoom: 0.5, showControls: r2, zoomStep: 0.1, children: jsx("div", { "aria-label": "Mermaid chart", className: s2("flex justify-center", n ? "size-full items-center" : null), dangerouslySetInnerHTML: { __html: I3 }, role: "img" }) }) });
};
...
//# sourceMappingURL=chunk-MAGV6XDF.js.map