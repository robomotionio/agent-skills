...
function emitSpa(contract, hash) {
  ...
  const header = [
    ...
    `// App: ${contract.name}`,
    "",
    definesFileRef ? 'import type { AppClient } from "@robomotion/apps-runtime";' : 'import type { AppClient, FileRef } from "@robomotion/apps-runtime";',
    "",
  ...
  return [header, types, actions].filter(Boolean).join("\n\n") + "\n";
}
function emitFlow(contract, hash) {
  ...
  for (const [name, action] of Object.entries(contract.actions)) {
    ...
    const paramsTs = schemaToTs(action.params, "");
    const resultTs = schemaToTs(action.result, "");
    if (action.params.type === "object" && action.params.properties) {
...
//# sourceMappingURL=chunk-DVAUVKNF.js.map